...
Use the Auth0 Dashboard to create and configure a Google social connection with the proper OAuth credentials. If you are following the [Call Other's APIs on User's Behalf](/ai/docs/get-started/call-others-apis-on-users-behalf) quickstart, you can skip the below steps and proceed to get the connection automatically set up using the **Configure Google Social Connection** step of the quickstart.
