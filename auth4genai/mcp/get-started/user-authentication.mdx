...
<Warning>
Because of DCR MCP Inspector will create one application in your tenant every time you run it because it doesn’t cache the application information to re-use it. Read [Securing an Open DCR Endpoint](/mcp/guides/registering-your-mcp-client-application#securing-an-open-dcr-endpoint) to learn strategies to mitigate risks with Open DCR.
</Warning>
...
When you perform the Quick OAuth Flow button, MCP Inspector performs all the steps in the OAuth Flow Progress section as outlined in the [MCP authorization flow with Auth0](/mcp/intro/user-authentication#mcp-authorization-flow-with-auth0).
