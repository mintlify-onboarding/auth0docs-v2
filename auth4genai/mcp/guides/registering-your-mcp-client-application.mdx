...
2. Use the search query:
   ```
   type:sapi AND description:*Create*client*
   ```
   This filters for successful Management API operations (`sapi`) related to client creation, which includes both manual and Dynamic Client Registration events.
...
**Advanced monitoring with the Management API:**

You can also retrieve DCR logs programmatically:

```shell wrap lines
# Get recent client creation events (last 24 hours)
curl --request GET \
  --url 'https://YOUR_TENANT/api/v2/logs?q=type:sapi%20AND%20description:*Create*client*' \
