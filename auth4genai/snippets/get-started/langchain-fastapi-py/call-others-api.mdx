...
- `backend/`: contains the backend code for the Web app, API written in Python using FastAPI and the LangGraph agent.
- `frontend/`: contains the frontend code for the Web app, written in React as a Vite SPA.

### Install backend packages

In the `backend` directory of your project, install the required packages using your preferred package manager, such as `uv`. You can follow the instructions here to [install uv](https://docs.astral.sh/uv/getting-started/installation/).
...
```
<Note>
  This will open the LangGraph Studio in a new tab, you can safely close it.
</Note>
...
In the `backend` directory of your project, add to your environment file the following variables:
...
Update your API route to pass the user session data including and federated access token to the AI agent, for example in `app/api/routes/chat.py`:
...
To implement, install the Auth0 AI Components for React to get the required UI components:
...
Want to see how it all comes together? Explore or clone the fully implemented sample application [here on GitHub](https://github.com/auth0-samples/auth0-ai-samples/tree/main/call-apis-on-users-behalf/others-api/langchain-fastapi-py).