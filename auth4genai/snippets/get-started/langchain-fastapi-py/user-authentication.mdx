...
In the root directory of your project, create the `.env.local` file and add the following variables:
...
- To set up first-party tool calling, complete the
  [Call your APIs on user's behalf](/call-your-apis-on-users-behalf) quickstart.
- To set up third-party tool calling, complete the
  [Call other's APIs on user's behalf](/call-others-apis-on-users-behalf) quickstart.
- To explore the Auth0 Next.js SDK, see the
  [Github repo](https://github.com/auth0/nextjs-auth0).
- [User Authentication for GenAI docs](/user-authentication).
ne):
