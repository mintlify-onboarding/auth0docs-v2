...
```bash wrap lines
npm install @auth0/ai-langchain@3 @langchain/core@0.3 @langchain/langgraph@0.3 @langchain/openai@0.6 langchain@0.3
```
...
export async function POST(
  ...
  { params }: { params: Promise<{ _path: string[] }> }
    ...
    if (endpoint.includes("/runs")) {
      requestBody = addAuthContext(body, user);
...
function addAuthContext(body: any, user: any) {
  return {
    ...body,
    config: {
      configurable: {
