...
```bash wrap lines
npm install @auth0/ai-langchain@3 @langchain/community@0.3 @langchain/core@0.3 @langchain/langgraph@0.3 @langchain/openai@0.6 langchain@0.3
```
...
Update the `/src/app/api/chat/[..._path]/route.ts` file with the following code. The `accessToken` will be passed to your LangGraph agent so we can use it from the Auth0 AI SDK to get Google access tokens from the server.

```ts src/app/api/chat/[..._path]/route.ts wrap lines highlight={48-54}
