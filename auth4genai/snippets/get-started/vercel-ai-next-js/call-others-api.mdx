...
```tsx src/lib/auth0.ts lines highlight={4-7}
//...
//... existing code
...
```ts src/lib/tools/google-calendar.ts lines
import { tool } from "ai";
...
```tsx src/components/auth0-ai/FederatedConnections/FederatedConnectionInterruptHandler.tsx lines
...
```tsx src/components/chat-window.tsx lines highlight={2,3,10-18,36-38}
//...
...
```ts src/app/api/chat/route.ts lines highlight={2-7,14,17,21,35-38,40-43}
//...
