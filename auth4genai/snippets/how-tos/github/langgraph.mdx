...
On the server side of your Next.js application you need to set up a route to handle the Chat API requests. This route will be responsible for forwarding the requests to the LangGraph API. Additionally, you must provide the `accessToken` to the Langchain's RunnableConfig from the authenticated user's session.

```typescript ./src/app/api/langgraph/[..._path]/route.ts wrap lines highlight={24-29,48-54}
