---
title: Auth0 APIs
mode: wide
description: Auth0 exposes the following APIs for developers to consume in their applications.
---

import {AuthenticationApiSection, ManagementApiSection, MyAccountApiSection} from "/snippets/api.jsx";

<div class="api_page_container">

<Card
  title="Authentication API"
  href="https://auth0.com/docs/api/authentication"
  arrow
/>
<AuthenticationApiSection/>


<Card
  title="Management API"
  href="https://auth0.com/docs/api/management/v2"
  arrow
/>
<ManagementApiSection/>

<Card
  title="MyAccount API"
  href="https://auth0.com/docs/api/myaccount"
  arrow
/>

<MyAccountApiSection/>

---

## Pagination

For Public Cloud tenants, Management API endpoints return a maximum of 50 results per response. When there are more results than can be returned in a response, include the `page` and `per_page` parameters in your request to retrieve more results.

See [Migrate to Management API v2 Endpoint Paginated Queries](/troubleshoot/product-lifecycle/past-migrations/migrate-to-paginated-queries) for information on updating your applications.

---

## Management API v1 (Deprecated)

The Management API v1 is deprecated and should not be used for new projects.

Management API v1 reached its End of Life on **July 13, 2020**. You may have been required to take action before that date to ensure no interruption to your service. Notifications were sent to customers that needed to complete this migration.

</div>