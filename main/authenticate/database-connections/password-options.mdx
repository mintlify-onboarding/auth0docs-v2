...
When using passwords for authentication, you should enforce the creation of unique passwords. A strong password policy will make it difficult, if not improbable, for a <Tooltip tip="Bad Actors: Entity (a person or group) that poses a threat to the business or environment with the intention to cause harm." cta="View Glossary" href="/glossary?term=bad+actor">bad actor</Tooltip> to guess a password through either manual or automated means.
...
The **Password Policy** settings page contains the ability to configure the [Password Strength Policy](/authenticate/database-connections/password-strength) as well as the following password options.
