---
description: Describes how to force a user to log out of applications using the
...
The Auth0 [RP-initiated logout endpoint](/authenticate/login/logout/log-users-out-of-auth0) works in one of two ways:

* Invalidates the Single Sign-on (SSO) cookie in Auth0. (The cookie still remains in the browser.)
* Signs the user out from the <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/glossary?term=identity+provider">identity provider</Tooltip> (IdP) (such as ADFS or Google).
