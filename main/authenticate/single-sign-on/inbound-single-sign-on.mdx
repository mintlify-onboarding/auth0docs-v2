...
* We also provide generic instructions to [configure Auth0 as a SAML service provider](/authenticate/protocols/saml/saml-sso-integrations/configure-auth0-saml-service-provider).
* [Configure Identity-Provider-Initiated Single Sign-On](/authenticate/protocols/saml/saml-sso-integrations/identity-provider-initiated-single-sign-on)

## Limitations

* Native applications can only use [Universal Login](/authenticate/login/auth0-universal-login).
* <Tooltip tip="OpenID: Open standard for authentication that allows applications to verify users' identities without collecting and storing login information." cta="View Glossary" href="/glossary?term=OpenID">OpenID</Tooltip> Connect (OIDC) does not support IdP-initiated SSO.
