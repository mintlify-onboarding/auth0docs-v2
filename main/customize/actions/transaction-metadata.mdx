---
description: Describes how to pass user or application metadata between login
...
Actions Transaction Metadata is currently in Early Access. To learn more, read [Product Release Stages](/troubleshoot/product-lifecycle/product-release-stages).

</Callout>

Actions Transaction Metadata stores, accesses, and/or shares, custom metadata within a [post-login](/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) Action for the duration of a transaction.
...
Before using transaction metadata in development, we recommend reviewing the limitations. To learn more, read [Actions Limitations](/customize/actions/limitations).

</Callout>

## How it works

Use the [post-login API object](/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object) `api.transaction.setMetadata` to set the key/value pair in order to store transaction metadata.

Use the [post-login Event object](/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object) `event.transaction.metadata` to access stored key/value pair in the same Action or subsequent Actions on the post-login trigger for a single execution.
...
Using Actions Transaction Metadata could cause a nominal additional latency. Any latency would be proportional to the metadata payload size and would be relevant when Action suspensions happen. For example, triggering <Tooltip tip="" cta="View Glossary" href="/glossary?term=MFA">MFA</Tooltip>, redirecting from Actions, or rendering Forms, could cause latency issues due to the need of reloading data from storage.
...
Serialize values as `strings` within [limits](/customize/actions/limitations#transaction-metadata).
...
Preserve transaction metadata during [redirects](/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions). Values are available when users continue the authentication flow.
...
[Render Forms using Actions](/customize/forms/render) and transaction metadata values would be preserved and available when users continue the authentication flow.

To learn more about using Forms with Actions, read [Render Forms using Actions](/customize/forms/render).
