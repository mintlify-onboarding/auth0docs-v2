...
You must configure your own email provider using a third-party service (such as Amazon SES, Mandrill, SendGrid, SparkPost, Mailgun, or a custom SMTP provider) to be able to customize your emails. To learn more, read [Configure External SMTP Providers](/customize/email/smtp-email-providers).
...
* Tenant-related information (defined in the [Tenant Settings](https://manage.auth0.com/#/tenant)):
  ...
  + `support_email`
  + `support_url`
* Multiple <Tooltip tip="Custom Domain: Third-party domain with a specialized, or vanity, name." cta="View Glossary" href="/glossary?term=custom+domains">custom domains</Tooltip> information:
...
The following email templates can be further customized for [Organizations](/manage-users/organizations/organizations-overview):
...
* **Invite User**: Received by the end-user when they are invited to an organization. Contains a link that redirects them to a custom invitation page. To learn more, see [Invite Organization Members](/manage-users/organizations/configure-organizations/invite-members).

When a user logs in through an [Organization](/manage-users/organizations/organizations-overview), the following additional variables are available:
...
Users will see the email address in the **From Address** field as the sender when receiving an email from Auth0. If you do not configure the **From Address** field for an email template, Auth0 will use the email address in the **From** field configured for your [Email Provider](/customize/email/smtp-email-providers).
...
* [Email Template Descriptions](/customize/email/email-templates/email-template-descriptions)
...
* [Use Liquid Syntax in Email Templates](/customize/email/email-templates/use-liquid-syntax-in-email-templates)