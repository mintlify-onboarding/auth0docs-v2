...
A review of the **Update user** action shows a syntax error in the `User ID` parameter, the correct syntax is `{{context.user.user_id}}`. Once the syntax is corrected, test the flow to confirm. To learn more about variable syntaxes, read [Variables](/customize/forms/variables).
...
If a field is configured as transient or when its value is masked with a [Helper function](/customize/forms/variables#helper-functions), the value is not available to debug.
