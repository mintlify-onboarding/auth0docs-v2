...
2. **Each site can have its own Application and share a database connection**
   ...
   1. Run the Setup Wizard steps to completion for the main site.
   2. Next, [manually create an Application for each site](/customize/integrations/cms/wordpress-plugin/configure-login-by-auth0) and add each one to the previously-created database connection.
...
This will create and configure an Application and a database connection plus data migration from your WordPress database. This requires an inbound connection from Auth0 servers and cannot be changed later without losing data. To learn more, read [User Migration in Login by Auth0 WordPress Plugin](/customize/integrations/cms/wordpress-plugin/user-migration-in-login-by-auth0).

::: warning
If you have more than one custom database connection in Auth0, you'll need to make sure that the user IDs are namespaced to avoid conflicts. This is done automatically for sites installing version 3.11.0 or later. If your connections are/were being created with an earlier version, read the troubleshooting steps in [User Migration in Login by Auth0 WordPress Plugin](/customize/integrations/cms/wordpress-plugin/user-migration-in-login-by-auth0).
:::
...
Now you're ready to [configure the application](/customize/integrations/cms/wordpress-plugin/configure-login-by-auth0).
...
3. Configure the application for your WordPress installation. To learn how to configure the application, read [Configure Login by Auth0](/customize/integrations/cms/wordpress-plugin/configure-login-by-auth0) and follow all sections through "Update Auth0 settings in WordPress".
