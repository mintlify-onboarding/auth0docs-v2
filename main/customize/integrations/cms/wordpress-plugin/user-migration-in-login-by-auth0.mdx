---
description: Describes the user migration feature of the Login by Auth0 WordPress
  Plugin
...
The easiest way to construct User Migration is to use the Setup Wizard when the plugin is first installed. To learn more about the process, read [Install Login by Auth0](/customize/integrations/cms/wordpress-plugin/install-login-by-auth0).
...
1. [Create and correctly configure an Application](/customize/integrations/cms/wordpress-plugin/configure-login-by-auth0), and create and activate an empty Database Connection for the Application. These can be the same as the ones created in the standard Setup Wizard process, or you can create these from scratch. To learn more about the Setup Wizard, read [Install Login by Auth0](/customize/integrations/cms/wordpress-plugin/install-login-by-auth0).
...
* If you do have data in Auth0 that needs to be kept, you can [use our User Import/Export Extension](/manage-users/user-migration/user-import-export-extension) to adjust the user data.
  ...
  3. Change all user IDs to add the namespace used when creating the new connection. User IDs should go from something like `auth0|123` to `auth0|Your-WP-Site-Name|123`. Adjust all other fields you need to follow the import schema. To learn more, read [Bulk User Import Database Schema and Examples](/manage-users/user-migration/bulk-user-import-database-schema-and-examples).
