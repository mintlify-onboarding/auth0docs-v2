...
---
The <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip> provides functionality that you can use to manage users of your application, including these types of tasks:

* Link separate user accounts from different providers, tying them to a single profile. To learn more, read [User Account Linking](/manage-users/user-accounts/user-account-linking).
* Unlink user accounts, returning them to separate identities.
* Update user metadata. To learn more, read [Understand How Metadata Works in User Profiles](/manage-users/user-accounts/metadata).
