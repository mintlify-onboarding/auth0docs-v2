---
description: Describes how to Salesforce as SAML identity provider for use with
  Auth0.
...
Configure Auth0 as a service provider to communicate with the Salesforce identity provider for <Tooltip tip="Single Sign-On (SSO): Service that, after a user logs into one applicaton, automatically logs that user in to other applications." cta="View Glossary" href="/glossary?term=SSO">SSO</Tooltip>.
...
When troubleshooting SSO, it is often helpful to capture an HTTP trace of the interaction and save it in a HAR file. To learn more, read [Generate and Analyze HAR Files](/troubleshoot/troubleshooting-tools/generate-and-analyze-har-files).
