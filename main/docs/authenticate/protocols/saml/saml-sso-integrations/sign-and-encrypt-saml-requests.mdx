...
* Auth0 as the SAML <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/glossary?term=identity+provider">identity provider</Tooltip> (for example, an application configured with the SAML Web App addon)
...
To learn how to get the private key and certificate formatted as a JSON string to use in the payload, see [Work with Certificates and Keys and Strings](/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings).
...
To learn how to turn the private key and certificate files into strings that you can use in a rule, see [Work with Certificates and Keys and Strings](/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings).
...
If Auth0 is the SAML identity provider, you can use [Actions](/customize/actions) to encrypt the SAML assertions it sends.
...
Once you get the certificate and public key files, you must [turn them into strings](/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings) to use them in an Action. The Action will look like this:
...
* [Work with Certificates and Keys as Strings](/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings)