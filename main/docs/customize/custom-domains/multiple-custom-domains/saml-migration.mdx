...
* Two verified custom domains configured in your Auth0 tenant (one for identity provider, one for service provider)
* Terraform CLI and `Node.js/npm` installed
* Cloudflare account with access to the domain hosting your custom domains.

## How it works
This migration strategy uses a smart reverse proxy to bridge the gap between your legacy custom domain and your new domain. This proxy, often a Cloudflare Worker, is deployed on the old domain to intercept the SAML authentication response sent by your external Identity Provider (IdP). 
