...
For some errors, the user may be sent to the [Generic Error Page](/authenticate/login/auth0-universal-login/error-pages) page instead of the Redirect To URL.
</Callout>

You can prevent sending the email addresses on the redirect links by updating the email templates using the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip>. For example:
...
* If the user is logged in through an [Organization](/manage-users/organizations/organizations-overview):
...
If a user requests a password change, they will receive a **Change Password** email that contains a URL link. When the user clicks on the link, a [Password Reset page](/customize/login-pages/classic-login/customize-password-reset-page) will be presented to enter the new password.
...
This email type is sent whenever Auth0 detects that the user is trying to access the application using a password that has been leaked by a third party. These emails are only set after enabling **<Tooltip tip="Breached Password Detection: Form of attack protection in which Auth0 notifies your users if they use a username/password combination that has been compromised in a data leak on a third-party website or app." cta="View Glossary" href="/glossary?term=Breached+Password+Detection">Breached Password Detection</Tooltip>** in the [Attack Protection](https://manage.auth0.com/#/security/attack-protection) section of the dashboard. To learn more, see [Breached Password Detection](/secure/attack-protection/breached-password-detection).
...
If a user is invited to become a member of an [Organization](/manage-users/organizations/organizations-overview), they will receive a **User Invitation** email that contains a URL link. When the user clicks on the link, they will be redirected to your default login route. To learn more, see [Configure Default Login Routes](/authenticate/login/auth0-universal-login/configure-default-login-routes).
...
To localize emails, you can set the `locale` to be sent in `user_metadata` when creating an invitation. To learn more, see [Customize Email Templates: Multilingual Email Templates](/customize/email/email-templates).

### Redirect results for user invitation email templates

You must [configure a default login route](/authenticate/login/auth0-universal-login/configure-default-login-routes), to which users will be sent after selecting the link to accept the invitation. When redirecting, Auth0 will include the following parameters:
...
The <Tooltip tip="Passwordless: Form of authentication that does not rely on a password as the first factor." cta="View Glossary" href="/glossary?term=Passwordless">Passwordless</Tooltip> Email is sent when a passwordless access is requested, either by code (the user receives a code that types in the application) or by a link (the user clicks on a link and is taken directly to the application).
