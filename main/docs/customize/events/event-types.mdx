...
`user.created` events are published each time a user is created. These events capture users created through a variety of channels, such as a signup page, the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip>, bulk user imports, SCIM, or just-in-time provisioning (JIT), for Social, Enterprise, or <Tooltip tip="Passwordless: Form of authentication that does not rely on a password as the first factor." cta="View Glossary" href="/glossary?term=Passwordless">Passwordless</Tooltip> connections.
...
* [Create an Event Stream](/customize/events/create-an-event-stream)
