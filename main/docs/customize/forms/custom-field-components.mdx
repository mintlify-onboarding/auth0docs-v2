...
To use custom fields you need to enable [Custom Domains](/customize/custom-domains). Rendering a form with a custom field outside a custom domain, will display an error.
...
Add key-value pairs to reference in the custom field source code. Key-value pairs can include form field [variables](/customize/forms/variables).
...
To accommodate complex validation requirements, you can use a [flow](/customize/forms/flows).
