...
To learn more, read [Custom Field Components for Forms](/customize/forms/custom-field-components).
...
By default, a router node has a single pass-through rule named **Default Case** that connects to other nodes. You can add additional rules based on a set of conditions that a variable must meet to then connect to other nodes. To learn more, read [Router.](/customize/forms/routers)
...
Flow nodes let you add and create custom logic and integration flows to your forms. To learn more, read [Flows.](/customize/forms/flows)
...
Start nodes are not visible to the user. It is where you can configure hidden field variables, such as user attributes when [rendering a form with an Action.](/customize/forms/render)
