...
When you no longer need Hook Secrets for a given Hook, you can delete them using either the Dashboard or the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip>.

Hook Secrets can also be imported and exported using the [Deploy Command-Line Interface (CLI) tool](/deploy-monitor/deploy-cli-tool).
