...
---
By integrating Auth0 with AWS, you'll allow your users to log in to AWS using any supported <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/glossary?term=identity+provider">identity provider</Tooltip>.

To configure AWS for <Tooltip tip="Single Sign-On (SSO): Service that, after a user logs into one applicaton, automatically logs that user in to other applications." cta="View Glossary" href="/glossary?term=SSO">SSO</Tooltip>, you need to complete the following steps:
...
To see an example of defining a server-side rule that assigns a role in an advanced use case, see the [Amazon API Gateway tutorial](/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-5).
...
1. To map an AWS role to a user, create a [rule](/customize/rules):
