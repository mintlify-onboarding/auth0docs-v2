...
Exclude caching on your server for all the URLs listed in the **Allowed Callback URLs** field in <AuthLink href="https://manage.auth0.com/#/applications/{yourClientId}/settings">Auth0 Dashboard > Applications > Applications > Settings</AuthLink> and test again. In addition, exclude caching the site URL (`/index.php` on a regular install) if it has an Auth0 URL parameter.
