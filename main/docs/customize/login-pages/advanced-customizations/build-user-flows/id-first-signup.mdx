...
* An Auth0 development tenant configured with [Universal Login](/authenticate/login/auth0-universal-login) and [custom domain](/customize/custom-domains).
* Enable [Identifier First Authentication](/authenticate/login/auth0-universal-login/identifier-first) on your tenant to render the ACUL Login ID screen.
* A development application or an [Auth0 SDK sample application](/quickstart/spa/react/interactive) on `localhost` for Auth0 authentication.
* A [database connection](/authenticate/database-connections) to authenticate users.
* An [Auth0 ACUL sample application](https://github.com/auth0-samples/auth0-acul-samples) to render the ACUL screens.

</Card>

By the end of this guide, you'll have an identity-first flow with customized Signup ID and Signup Password screens. To learn more, read the [Getting Started guide](/customize/login-pages/advanced-customizations/getting-started) and visit the [SDK reference guide](/customize/login-pages/advanced-customizations/getting-started/sdk-quickstart).
