...
If you are returning custom error codes from a [rule](/customize/rules) or a custom database script, you can handle custom errors:

* In your application's redirect URL by reading the `error` and `error_mesage` query string parameters.
* By redirecting the user back to your hosted pages with a custom error message and displaying the message with a [flash message](/libraries/lock/lock-api-reference).
