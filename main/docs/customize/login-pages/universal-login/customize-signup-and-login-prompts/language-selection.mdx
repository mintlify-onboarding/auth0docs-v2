...
* Use [Universal Login](/authenticate/login/auth0-universal-login).

  + This feature is not available for custom login pages.
* Configure a [custom domain](/customize/custom-domains).
* Enable the languages you plan to add to your prompts in your Auth0 tenant.
  ...
  + On the Auth0 Dashboard, navigate to [Settings > General > Languages](https://manage.auth0.com/#/tenant/general) and enable one or more desired languages.
    ...
    - For more information, review [Universal Login Internationalization](/customize/internationalization-and-localization/universal-login-internationalization).
...
To get started, add the following script to your [Universal Login page template](/customize/login-pages/universal-login/customize-templates) to facilitate language selection.
...
To add language selection to a specific prompt, use the Auth0 <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip> to configure custom prompt partials with the following parameters:
