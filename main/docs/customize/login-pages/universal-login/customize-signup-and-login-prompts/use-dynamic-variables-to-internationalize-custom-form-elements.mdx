...
---
You can render [Sign-Up Prompt Customizations](/customize/login-pages/universal-login/customize-signup-and-login-prompts) differently depending on contextual data. It uses the `locale` variable to conditionally render form inputs and define validation behavior.

The following use case works with the `locale` variable to render a Terms of Service message and checkbox. Any variable exposed to [Page Templates](/customize/login-pages/universal-login/customize-templates#available-variables) can be substituted.

### Prerequisites

* Tenant has a verified [Custom Domain](/customize/custom-domains)
* Tenant has a Page Template set

### Enable fr and es locales

Go to your <Tooltip tip="Auth0 Dashboard: Auth0's main product to configure your services." cta="View Glossary" href="/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>, then navigate to Settings > General and enable the locales:
...
Use <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip> to add a template partial to the `form-content-end` signup prompt <Tooltip tip="Universal Login: Your application redirects to Universal Login, hosted on Auth0's Authorization Server, to verify a user's identity." cta="View Glossary" href="/glossary?term=Universal+Login">Universal Login</Tooltip> Prompt container. In the case where the locale is `es` or `fr`, you may want to render a Terms of Service message and a checkbox.
