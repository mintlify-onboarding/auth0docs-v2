...
Values stored in this way will have lifetimes of up to the specified `ttl` or `expires_at` values. If no lifetime is specified, a default of lifetime of 15 minutes will be used. Lifetimes may not exceed the maximum duration listed at [Actions Cache Limits](/customize/actions/limitations).
...
We will log an error for this event [`(fn)`](/deploy-monitor/logs/log-event-type-codes), but won't be sending it again to the action in the future.
...
We will log an error for this event [`(fn)`](/deploy-monitor/logs/log-event-type-codes), but we will retry it up to 5 times in the next few minutes.
