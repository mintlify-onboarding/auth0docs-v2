...
**Scenario: Custom phone provider configured using Terraform is not taking effect, and the <Tooltip tip="Auth0 Dashboard: Auth0's main product to configure your services." cta="View Glossary" href="/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> displays a different Action.**
...
To reset the custom phone provider, navigate to [Auth0 Dashboard > Branding](/$manage_url#/branding) and select **Phone Provider**. Then, select the **Reset** button to restore your custom provider to its default