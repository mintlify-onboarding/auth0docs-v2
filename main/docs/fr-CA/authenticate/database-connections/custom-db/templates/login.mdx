...
<table class="table">
  <thead>
    <tr>
      <th>
        <strong>Champ d’événement de journal de locataire</strong>
      </th>
      <th>
...
<table class="table">
  <thead>
    <tr>
      <th>
        <strong>Champ d’événement du journal des locataires</strong>
      </th>
      <th>
        <strong>Valeur</strong>
      </th>
...
function login(email, password, callback) {
  ...
  connection.on("connect", function (err) {
    ...
    getMembershipUser(email, function (err, user) {
      if (err || !user || !user.profile || !user.password)
...
function login(email, password, callback) {
  ...
  var query = "SELECT Id, Email, Password " + "FROM dbo.Users WHERE Email = @Email";
  connection
    .on("debug", function (text) {
