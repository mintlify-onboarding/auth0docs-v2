...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        L’utilisateur n’était pas connecté à Auth0, donc l’authentification silencieuse n’est pas possible. Cette erreur
        peut se produire en fonction de la configuration des paramètres{" "}
        <strong>Log In Session Management (Gestion de session de connexion)</strong> au niveau du locataire; en
        particulier, elle peut se produire après la période définie dans le paramètre{" "}
        <strong>Require log in after (Exiger la connexion après)</strong>. Consultez{" "}
        <a href="/docs/fr-CA/dashboard/guides/tenants/configure-session-Lifetime-settings">
    ...
    <tr>
      ...
      <td>
        L’utilisateur était connecté à Auth0 et a autorisé l’application, mais doit être redirigé ailleurs avant que
        l’authentification puisse être terminée; par exemple, lors de l’utilisation d’une{" "}
        <a href="/docs/fr-CA/rules/redirect">règle de redirection</a>.
      </td>
    </tr>
  </tbody>
</table>
