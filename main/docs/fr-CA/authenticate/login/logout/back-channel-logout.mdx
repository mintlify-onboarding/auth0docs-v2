...
<Frame>
  ![Workflow for back-channel
  logout](/docs/images/fr-ca/cdy7uua7fh8z/5jQ5HogFNeD9tqUGEJuJHE/663f1f52b285f7019fd1157f5a26caf0/2023-09-29_09-32-00.png)
</Frame>
...
<Frame>
  ![Back-channel logout multiple app use
  case](/docs/images/fr-ca/cdy7uua7fh8z/54mbNobsvLec0A2tz0DXUG/99ce79c9e93ffe526472d6250373778c/2023-06-20_09-39-12.png)
</Frame>
...
router.post("/backchannel-logout", requiresValidLogoutToken, function (req, res, next) {
  ...
  deleteUserSessions(req.app.locals.sessionStore, req.logoutToken.sub, req.logoutToken.sid);
...
async function requiresValidLogoutToken(req, res, next) {
  ...
  try {
    ...
    if (!payload.sub && !payload.sid) {
      res.status(400).send("Error: Logout token must contain either sub claim or sid claim, or both");
    }
    ...
    if (!payload.events["http://schemas.openid.net/event/backchannel-logout"]) {
      res.status(400).send("Error: Logout token must contain events claim with correct schema");
    }

    // Verify that the Logout token does not contain a nonce claim.
    if (payload.nonce) {
      res.status(400).send("Error: Logout token must not contain a nonce claim");
    }
...
<Frame>
  ![Logout Token
  Store](/docs/images/fr-ca/cdy7uua7fh8z/2zTtcie1d0fjiSuxZ4huyV/dbcf45fb68a045bd1b1918315f8025f7/image__21_.png)
</Frame>
