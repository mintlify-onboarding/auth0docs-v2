...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <em>Chaîne</em>. Une explication lisible par l’homme du rejet de la connexion. Elle est envoyée en tant que{" "}
        <code>error_description</code> à l’application qui a initié la demande.
      </td>
    </tr>
  </tbody>
</table>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <p>
          <em>Tableau</em>. Tableau d’objets qui inclut le champ <code> type</code>. <code> type</code> est une chaîne
          utilisée pour que l’utilisateur puisse choisir un facteur MFA lorsqu’il est confronté à un défi-réponse.
        </p>
        <p>Les valeurs prises en charge sont les suivantes :</p>
        <ul>
          <li>`otp`</li>
          ...
          <li>
            ...
            <ul>
              <li>
                <code>otpFallback</code>Si la valeur est <code>false</code>, l’utilisateur ne peut pas utiliser l’option
                de mot de passe unique du facteur de notification push.{" "}
              </li>
            </ul>
          </li>
          <li>
            ...
            <ul>
              <li>
                <code>preferredMethod: voice</code>
              </li>
              <li>
                <code>preferredMethod: sms</code>
              </li>
              <li>
                <code>preferredMethod: both</code>
              </li>
            </ul>
          </li>
          <li>
            <code>webauthn-platform</code>
          </li>
          <li>
            <code>webauthn-roaming</code>
          </li>
        </ul>
      </td>
    </tr>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <p>
          <em>Tableau</em>. Tableau d’objets qui inclut le champ <code> type</code>. <code> type</code> est une chaîne
          utilisée pour spécifier le facteur MFA par défaut utilisé lors de l’inscription de l’utilisateur.
        </p>
        <p>Les valeurs prises en charge sont les suivantes :</p>
        <ul>
          <li>
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td>
        <code>options.expires_at</code>
      </td>
      <td>
        <p>
          <em>Nombre facultatif</em>. Le temps d’expiration absolu en millisecondes depuis l’Unix epoch. Bien que les
          enregistrements mis en cache puissent être supprimés plus tôt, ils ne resteront jamais au-delà du{" "}
          <code>expires_at</code> fourni.
        </p>
        <p>
          <em>Remarque :</em> Cette valeur ne doit pas être fournie si une valeur a également été fournie pour{" "}
          <code>ttl</code>. Si les deux options sont fournies, c’est la première des deux échéances qui sera retenue.
        </p>
      </td>
    </tr>
    <tr>
      <td>
        <code>options.ttl</code>
      </td>
      <td>
        <p>
          <em>Nombre facultatif</em>. Valeur de la durée de vie de cette entrée de cache en millisecondes. Bien que les
          enregistrements mis en cache puissent être supprimés plus tôt, ils ne resteront jamais au-delà du{" "}
          <code>ttl</code> fourni.
        </p>
        <p>
          <em>Remarque :</em> Cette valeur ne doit pas être fournie si une valeur a également été fournie pour{" "}
          <code>expires_at</code>. Si les deux options sont fournies, c’est la première des deux échéances qui sera
          retenue.
        </p>
      </td>
    </tr>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <p>
          <em>Chaîne</em>. Le nom du fournisseur multifacteur à utiliser ou la valeur <code>any</code> à utiliser de
          n’importe lequel des fournisseurs configurés.
        </p>
        <p>Les valeurs prises en charge sont les suivantes :</p>
        <ul>
          <li>
            <code>any</code> Utiliser l’un des défis configurés.
          </li>
          <li>
            <code>duo</code> Utiliser le fournisseur multifacteur Duo.
          </li>
          <li>
            <code>google-authenticator</code> Utiliser le fournisseur Google Authenticator.
          </li>
          <li>
            <code>guardian</code> Utiliser le fournisseur Guardian.
          </li>
          <li>
            <code>none</code> N’utiliser aucun des défis configurés pour empêcher le déclenchement du flux MFA.
          </li>
        </ul>
      </td>
    </tr>
    ...
    <tr>
      <td>
        <code>options.allowRememberBrowser</code>
      </td>
      <td>
        <em>Booléen facultatif</em>. Lorsque le fournisseur est défini sur <code>google-authenticator</code> ou{" "}
        <code>duo</code>, l’utilisateur est invité à s’authentifier avec la MFA une fois tous les 30 jours. Lorsque le
        fournisseur est <code>guardian</code>, l’invite MFA affiche la case à cocher pour l’inscription pour que les
        utilisateurs choisissent de s’inscrire ou non. La valeur par défaut est <code>false</code>. Pour en savoir plus,
        lisez{" "}
        <a href="/docs/secure/multi-factor-authentication/customize-mfa ">
    ...
    <tr>
      ...
      <td>
        <p>
          <em>Objet facultatif</em>. Options supplémentaires pour configurer le défi, disponibles uniquement pour le
          fournisseur <code>duo</code>.
        </p>
        <p>Les options prises en charge sont les suivantes :</p>
        <ul>
          <li>
            <code>host</code> <em>Chaîne</em>. Il s’agit de la valeur du nom d’hôte de l’API de votre compte Duo.
          </li>
          <li>
            <code>ikey</code> <em>Chaîne</em>. Il s’agit de la valeur de l’ID client (anciennement clé d’intégration) de
            votre compte Duo.
          </li>
          <li>
            <code>skey</code> <em>Chaîne</em>. Il s’agit de la valeur du secret du client (anciennement clé secrète) de
            votre compte Duo.
          </li>
          <li>
            <code>username</code> <em>Chaîne facultative</em>. Utiliser un attribut du profil comme nom d’utilisateur
            dans DuoSecurity. Cette fonction est également utile si vos utilisateurs sont déjà inscrits dans Duo.
          </li>
        </ul>
      </td>
    </tr>
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <em>Toute valeur</em>. La valeur de la propriété des métadonnées. Cette valeur peut être définie sur{" "}
        <code>null</code> pour supprimer la propriété des métadonnées.
      </td>
    </tr>
  </tbody>
</table>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <em>Options</em>. Configurer la manière dont les données sensibles sont encodées dans les paramètres de requête
        de l’URL résultante.
      </td>
    </tr>
    <tr>
      <td>
        <code>options.expiresInSeconds</code>
      </td>
      <td>
    ...
    <tr>
      <td>
        <code>options.payload</code>
      </td>
      <td>
        <em>Options</em>. Données destinées à être transmises à la cible de la redirection et dont l’authenticité et
        l’intégrité doivent pouvoir être prouvées.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <em>Chaîne</em>. Un secret qui sera utilisé pour signer un JWT partagé avec la cible de la redirection. La
        valeur secrète doit être stockée en tant que secret et récupérée à l’aide de{" "}
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <em>Options</em>. Un objet représentant des paramètres de chaîne de requête supplémentaires qui doivent être
        ajoutés à l’URL de redirection.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <em>Options</em>. Paramètres supplémentaires de la chaîne de requête qui doivent être ajoutés à l’URL de
        redirection.
      </td>
    </tr>
  </tbody>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <em>Options</em>. Options pour récupérer les données codées dans un jeton JWT transmis au point de terminaison
    ...
    <tr>
      <td>
        <code>options.tokenParameterName</code>
      </td>
      <td>
        <em>Chaîne</em>. Le nom du paramètre de la requête ou du corps qui a été envoyé au point de terminaison
        `/continue`. (par défaut <code>session_token</code>)
      </td>
    </tr>
  </tbody>
</table>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>Chaîne. L’ID de la règle à vérifier.</td>
    </tr>
  </tbody>
</table>
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <em>SAMLValue</em>. La valeur de l’assertion SAML. Cela peut être défini sur <code>null</code> pour supprimer la
...
<table class="table">
  ...
  <tbody>
    <tr>
      <td>
        <code>passthroughClaimsWithNoMapping</code>
      </td>
      <td>
...
<table class="table">
  ...
  <tbody>
    <tr>
      <td>
        <code>mapUnknownClaimsAsIs</code>
      </td>
      <td>
...
<table class="table">
  ...
  <tbody>
    <tr>
      <td>
        <code>nameIdentifierFormat</code>
      </td>
      <td>
...
<table class="table">
  ...
  <tbody>
    <tr>
      <td>
        <code>nameIdentifierProbes</code>
      </td>
      <td>
...
<table class="table">
  ...
  <tbody>
    <tr>
      <td>
        <code>includeAttributeNameFormat</code>
      </td>
      <td>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <code>rsa-sha256</code> | <code>rsa-sha1</code>. <code>rsa-sha1</code> ne devrait pas être utilisé. C’est
        dangereux.
      </td>
    </tr>
  </tbody>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <p>
          Cette méthode définit le délai d’inactivité de la session pour l’interaction en cours. Si la méthode n’est pas
          réappliquée, les interactions ultérieures réussies remplaceront le délai d’inactivité à l’aide des paramètres
          de délai d’inactivité de la session.
        </p>
      </td>
    </tr>
  </tbody>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <em>Chaîne</em>. Une explication lisible par l’être humain du rejet du jeton d’actualisation. Elle est envoyée
        en tant que error_description à l’application qui a initié la demande.
      </td>
    </tr>
  </tbody>
</table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <p>
          Cette méthode définit le délai d’inactivité du jeton d’actualisation pour l’interaction en cours. Si la
          méthode n’est pas réappliquée, les interactions ultérieures réussies remplaceront le délai d’inactivité à
          l’aide des paramètres de délai d’inactivité du jeton d’actualisation.
        </p>
      </td>
    </tr>
  </tbody>
