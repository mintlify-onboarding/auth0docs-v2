...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Jetons d’accès à Management API avec le{" "}
        <dfn id="react-containers-DefinitionTooltip-1">
          <span class="tooltip-portal-underlined-word" data-react-universal-portal="">
            scope
          </span>
        </dfn>{" "}
        `update:hooks`. Pour en savoir plus, lisez [Jetons d’accès à Management API] (/api/management/v2/tokens).
      </td>
    </tr>
    <tr>
      <td>`HOOK_SECRET_NAME`</td>
      <td>
        Nom(s) du (des) secret(s) que vous souhaitez supprimer de l’appel spécifié. Ce point de terminaison accepte un
        tableau de noms de secrets à supprimer.
      </td>
    </tr>
  </tbody>
</table>
