...
5. Sous **Méthodes d’authentification**, sélectionnez **Clé privée JWT**.
   ...
   <Frame>
     ![Configuration de l’authentification JWT Private Key (Clé privée JWT) - Instructions pour le Auth0 Dashboard
     (Tableau de bord
     Auth0)](/docs/images/fr-ca/cdy7uua7fh8z/33kfi48tkbMIOQJ8PBxj76/3a37d4906b80c31d5427b852f56cdb00/Default_App_-_Creds_-_French.png)
   </Frame>
...
3. Choisissez l’identifiant que vous voulez mettre à jour et sélectionnez **Edit Credential (Modifier l’identifiant)**.
   ...
   <Frame>
     ![Dashboard - Applications - Application Settings - Credentials - update expiry
     date](/docs/images/fr-ca/cdy7uua7fh8z/6858PTDkNCRZiIqtBVmciZ/98cad98076e61ac1a3a9d0620dbc99dc/Demo_Key2_-_French.png)
   </Frame>
...
<table class="table">
  ...
  <tbody>
    <tr>
      <td>`algorithm`</td>
      <td>
        Algorithme utilisé pour signer les assertions. Les valeurs prises en charge sont RS256, RS384 et PS256. S’il
        n’est pas précisé, l’algorithme sera par défaut RS256.
      </td>
    </tr>
    <tr>
      <td>`clientName`</td>
    ...
    <tr>
      <td>`expires_at`</td>
      <td>
        Facultatif. Date d’expiration de l’authentifiant au format ISO 8601. Par exemple, « 2020-08-20T19:10:06.299Z ».
        Une fois la date d’expiration passée, l’authentifiant n’est plus valide.
      </td>
    </tr>
    <tr>
      <td>`managementApiAccessToken`</td>
      <td>
        <a href="/docs/fr-CA/secure/tokens/access-tokens/management-api-access-tokens">
          Jeton d’accès pour Management API
        </a>{" "}
        avec la permission `create:credentials`.
      </td>
    </tr>
    <tr>
      <td>`pem`</td>
    ...
    <tr>
      ...
      <td>
        Facultatif. Un booléen qui indique à Auth0 d’analyser la date d’expiration lorsqu’un certificat lui est fourni.
        Si aucun certificat n’est fourni , Auth0 renverra une erreur. `parse_expiry_from_cert` et `expires_at` sont
        aussi mutuellement exclusifs. Dans ce cas, Auth0 renverra une erreur.
      </td>
    </tr>
  </tbody>
</table>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        Algorithme utilisé pour signer les assertions. Les valeurs prises en charge sont RS256, RS384 et PS256. S’il
        n’est pas spécifié, l’algorithme par défaut est RS256.
      </td>
    </tr>
    <tr>
      <td>`clientId`</td>
    ...
    <tr>
      ...
      <td>
        <a href="/docs/fr-CA/secure/tokens/access-tokens/management-api-access-tokens">
          Jeton d’accès à Management API
        </a>{" "}
        avec la permission `create:credentials`.
      </td>
    </tr>
    ...
    <tr>
      <td>`expires_at`</td>
      <td>
        Facultatif. Date d’expiration de l’identifiant au format ISO 860. Par exemple, `2020-08-20T19:10:06.299Z`. Une
        fois la date d’expiration passée, l’identifiant n’est plus valide.
      </td>
    </tr>
    <tr>
      <td>`parse_expiry_from_cert`</td>
      <td>
        Facultatif. Un booléen qui indique qu’Auth0 doit analyser l’expiration lorsqu’un certificat est fourni. Si aucun
        certificat n’est fourni, Auth0 renvoie une erreur. `parse_expiry_from_cert` et `expires_at` sont mutuellement
        exclusifs. Dans ce cas, Auth0 va retourner une erreur.
      </td>
    </tr>
  </tbody>
</table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        avec les permissions `update:client` et `update:credentials`.
      </td>
    </tr>
    <tr>
      <td>`credentialId`</td>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        avec les permissions `update:client`
      </td>
    </tr>
    <tr>
