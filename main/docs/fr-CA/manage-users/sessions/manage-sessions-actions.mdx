...
exports.onExecutePostLogin = async (event, api) => {
  ...
  if (sessionInitialAsn && sessionCurrentAsn && sessionInitialAsn != sessionCurrentAsn) {
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  if (sessionInitialIp && sessionCurrentIp && sessionInitialIp != sessionCurrentIp) {
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  const idle_organization_lifetime = event.organization?.metadata?.idle_session_timeout;
  ...
  if (event.session?.id && idle_organization_lifetime) {
    ...
    api.session.setIdleExpiresAt(current_time + Number(idle_organization_lifetime));
  }
};
```
