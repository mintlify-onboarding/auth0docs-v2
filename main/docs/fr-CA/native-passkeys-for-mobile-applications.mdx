...
2. Votre application lance ensuite le défi-réponse d’inscription en appelant le point de terminaison [Demander un défi-réponse d’inscription](/docs/fr-CA/native-passkeys-api#request-signup-challenge) d’Auth0 Authentication API :
   ...
   <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
     * Si vous ne spécifiez pas de `realm`, le répertoire par défaut de votre locataire est utilisé. * Par défaut, le
     courriel est l’identifiant requis. Si vous avez activé [Identifiants
     flexibles](/docs/fr-CA/authenticate/database-connections/activate-and-configure-attributes-for-flexible-identifiers)
     pour votre connexion de base de données, vous pouvez utiliser une combinaison de `email`, `phone_number`, ou
     `username` à la place. Ces options peuvent être obligatoires ou facultatives et doivent correspondre à votre
     configuration d’identificateur flexible.
   </Callout>
