...
>
  Auth0 Dashboard
</Tooltip> permet de essayer avec `TRY` une règle à des fins de test et de débogage. Cette fonction permet de définir des
objets `user` et `context` fictifs, qui sont ensuite transmis à la règle dans le cadre de son exécution. Le résultat de la
règle (y compris la journalisation de la console) est affiché à la fin de l’exécution. Bien qu’il s’agisse d’un moyen immédiat
de tester une règle à l’unité, il s’agit d’une approche très manuelle, qui ne permet pas de tirer parti d’outils de test
automatisés tels que [Mocha](https://mochajs.org/) ou [rewire](https://www.npmjs.com/package/rewire).
