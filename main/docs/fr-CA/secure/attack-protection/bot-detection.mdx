...
2. Dans la section **Détection**, activez le bouton bascule.
   ...
   <Frame>
     ![Section Détection de l’écran Protection contre les
     attaques](/docs/images/fr-ca/cdy7uua7fh8z/3q0HlAm0m0aLilEOAhQ81h/a6669bf2bb063e860b80ddb9bfbabc22/Bot_Detection_-_French.png)
   </Frame>
...
3. Dans la section **Réponse**, choisissez une réponse à la détection de robots.
   ...
   <Frame>
     ![Dashboard - Protection contre les attaques - Détection des
     robots](/docs/images/fr-ca/cdy7uua7fh8z/3vuVvc01mfCPdc70AU9Oq1/b5d5237e0e8efc048eee5bc03b359d57/Bot_Detection_Options.png)
   </Frame>
...
<Frame>
  ![Auth0 Dashboard > Security (Sécurité) > Protection contre les attaques pour accéder à ce
  curseur](/docs/images/fr-ca/cdy7uua7fh8z/6HSMEXmDHVks6aUH0FUTPo/88890090e3e357a627b095c12ee06daa/Bot_Detection_Levels_-_French.png)
</Frame>
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td>
        Connexion Classique (page de connexion personnalisée à l’aide du modèle <strong>Lock</strong> )
      </td>
      <td>Pris en charge si vous utilisez la trousse SDK ‘lock.js’ version 12.4.0 ou ultérieure.</td>
    </tr>
    <tr>
      <td>
        Connexion Classique (page de connexion personnalisée à l’aide du modèle <strong>Custom Login Form</strong> )
      </td>
      <td>
        Pris en charge si vous utilisez la trousse SDK ‘auth0.js’ version 9.24 ou ultérieure et que vous améliorez votre
        code pour gérer un défi CAPTCHA ou reCAPTCHA.
      </td>
    </tr>
    <tr>
      <td>Applications natives</td>
      <td>
        Prise en charge si vous utilisez l’une des trousses SDK suivantes :
        <ul>
          <li>`Auth0.swift` version 1.28.0+</li>
          <li>`Auth0.Android` version 1.25.0+</li>
          <li>`Lock.Swift` version 2.19.0+</li>
          <li>`Lock.Android` version 2.22.0+</li>
        </ul>
      </td>
    </tr>
    <tr>
      ...
      <td>
        Prise en charge dans une capacité limitée. Réponse de détection de robots tel que CAPTCHA nécessite un flux
        interactif et n’est donc pas pris en charge. Si la trousse SDK renvoie l’erreur `requires_verification’, vous
        devez déclencher un processus de connexion sur le Web pour que l’utilisateur s’authentifie.
      </td>
    </tr>
    <tr>
      <td>
        Flux non hébergés par Auth0 à l’aide des trousses SDK ‘lock.js’ ou ‘auth0.js’, qui effectue une authentification
        croisée des origines (point de terminaison ‘co/authenticate’)
      </td>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        Pris en charge si la connexion utilise un flux de connexion compatible comme décrit dans le tableau Limitations
        de flux.
      </td>
    </tr>
    <tr>
