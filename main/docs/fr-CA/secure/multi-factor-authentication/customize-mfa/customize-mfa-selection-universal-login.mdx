...
exports.onExecutePostLogin = async (event, api) => {
  ...
  // if available
  if (event.user.phone_number && event.user.phone_verified && availableFactors.includes("phone")) {
...
```js lines
exports.onExecutePostLogin = async (event, api) => {
  api.authentication.challengeWith({ type: "otp" }, { additionalFactors: [{ type: "email" }] });
};
```
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  api.authentication.challengeWith({ type: "otp" }, { additionalFactors: enrolledFactors });
};
```
