   ...
   <table class="table">
     ...
     <tbody>
       ...
       <tr>
         <td>`amr`</td>
         <td>
           Si `amr` n’existe pas dans la charge utile ou ne contient pas la valeur `mfa`, l’utilisateur ne s’est pas
           connecté avec la MFA. Si `amr` existe dans la charge utile et contient la valeur `mfa`, l’utilisateur s’est
           connecté avec la MFA.
         </td>
       </tr>
     </tbody>
   </table>
