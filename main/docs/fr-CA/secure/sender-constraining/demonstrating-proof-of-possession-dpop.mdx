...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Use an extension grant similar to the Resource Owner Password Grant that includes the ability to indicate a
        specific realm
      </td>
    </tr>
    <tr>
...
<Frame>
  ![](/docs/images/fr-ca/cdy7uua7fh8z/XoEV4y12QtnGwBPCiFbep/6744ab830ab2119664463f8c52fe6b02/Screenshot_2025-07-28_at_11.15.42â__AM.png)
</Frame>
...
<Frame>
  ![](/docs/images/fr-ca/cdy7uua7fh8z/7sonusMpvBMP0fDS6OkXSA/7cd0d50ffc44167f52f7e29d7f723d4a/Screenshot_2025-07-28_at_11.17.43â__AM.png)
</Frame>
...
<table class="table">
  <thead>
    <tr>
      <th>
        <strong>DPoP Proof JWT Claim</strong>
      </th>
      <th>
        <strong>Description</strong>
      </th>
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td>`htu`</td>
      <td>
        The HTTP URI of the request the DPoP Proof JWT is for, without the fragment and query parameters. For example:
        `https://api.example.com/data?param=1#section1` becomes `https://api.example.com/data`.
      </td>
    </tr>
    <tr>
      <td>`iat`</td>
...
const response = await fetch("https://[TENANT]/oauth/token", {
  ...
  body: new URLSearchParams({
    grant_type: "...",
    client_id: "...", // Other body parameters here
  }),
  headers: {
    "Content-Type": "application/x-www-form-urlencoded", // Add the DPoP header
    dpop: dpopHeader,
  },
});
...
if (response.status === 400 && result.error === "use_dpop_nonce") {
  ...
  console.log("Received nonce:", nonce); // Retry with the nonce
  ...
  return jwt.sign(
    {
      ...
      htu: url,
      iat: Date.now() / 1000,
      nonce, // Optionally include an `ath` claim containing an access token hash
...
const userInfoResponse = await fetch("https://[TENANT]/userinfo", {
  ...
  headers: {
    ...
    Authorization: `DPoP ${result.access_token}`, // Include a DPoP header, this time with access token hash
