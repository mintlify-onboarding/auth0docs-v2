...
<Tooltip href="/docs/fr-CA/glossary?term=id-token" tip="Jeton d’ID Identifiant conçu pour le client lui-même, et non pour l’accès à une ressource." cta="Voir le glossaire">ID tokens</Tooltip> follow the [JSON Web Token (JWT)](/docs/fr-CA/secure/tokens/json-web-tokens) standard, which means that their
basic structure conforms to the typical [JWT Structure](/docs/fr-CA/secure/tokens/json-web-tokens/json-web-token-structure),
and they contain standard [JWT Claims](/docs/fr-CA/secure/tokens/json-web-tokens/json-web-token-claims) asserted about the
token itself.
