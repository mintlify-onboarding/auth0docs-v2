   ...
   <table class="table">
     ...
     <tbody>
       ...
       <tr>
         ...
         <td>
           La période d’expiration par défaut d'un jeton d’actualisation, lorsque la{" "}
           <strong>Rotation des jetons d’actualisation</strong> est activée, est de 30 jours (2 592 000 secondes). Vous
           pouvez configurer jusqu’à 1 an (31 557 600 secondes).{" "}
           <strong>La durée de vie ne s’étend pas lorsque les jetons sont renouvelés.</strong>
         </td>
       </tr>
       <tr>
         <td>`leeway`</td>
         <td>
           Autoriser le même jeton d’actualisation à être utilisé pendant la période de temps pour tenir compte des
           problèmes de concurrence de réseau potentiels qui invalideraient autrement le jeton si le client tentait de
           réessayer en utilisant le même jeton d’actualisation. Par défaut, la marge par défaut est désactivés.
