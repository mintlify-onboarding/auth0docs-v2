...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        L’ID client de votre application. L’application doit correspondre à celle pour laquelle le jeton d’actualisation
        a été émis.
      </td>
    </tr>
    <tr>
      <td>`client_secret`</td>
      <td>
        Le secret client de votre application. Requis pour les{" "}
        <a href="/docs/fr-CA/applications/concepts/app-types-confidential-public#confidential-applications">
...
<table class="table">
  ...
  <tbody>
    <tr>
      <td>200</td>
      <td>
        Le jeton d’actualisation est révoqué, n’existe pas ou n’a pas été délivré à l’application qui a fait la demande
        de révocation. Le corps de la réponse est vide.
      </td>
    </tr>
    <tr>
      <td>400</td>
    ...
    <tr>
      <td>401</td>
      <td>
        La requête n’est pas autorisée (` "error" : "invalid_request") : "invalid_client"`). Vérifiez que les
        informations d’identification de l’application (`client_id` et `client_secret`) sont présentes dans la requête
        et qu’elles ont des valeurs valides.
      </td>
    </tr>
  </tbody>
</table>
