...
3. À côté de la connexion SAML, cliquez sur **Settings (Paramètres)** (r](/docs/images/par l’icône d’engrenage).
   ...
   <Frame>
     ![Dashboard Authentication Enterprise SAML Connection
     Settings](/docs/images/fr-ca/cdy7uua7fh8z/6s8p4aMck06YnPStr493sA/86cf45f919a2b75376a5ee6474b71cb1/2025-02-27_15-56-22.png)
   </Frame>
...
<table class="table">
  ...
  <tbody>
    <tr>
      <td>Destination</td>
      <td>
        Vérifiez que la destination de l’assertion SAML est le bon locataire Auth0 et la bonne connexion (`https://{TENANT}.auth0.com/login/callback?connection={CONNECTION}`).
      </td>
    </tr>
    <tr>
      <td>Status Field (Champ d’état)</td>
      <td>
        Ce champ doit indiquer la réussite. (`<samlp:StatusCodeValue="urn:oasis:names:tc:SAML:2.0:status:Success">`).
      </td>
    </tr>
    <tr>
      <td>Destinataire</td>
      <td>
        Vérifiez que l’élément Méthode `&lt;saml:SubjectConfirmation` contient le bon locataire et la bonne connexion
        dans le champ “Recipient” (`https://{TENANT}.auth0.com/login/callback?connection={CONNECTION}`).
      </td>
    </tr>
    <tr>
      <td>Public</td>
      <td>
        Vérifiez que le champ de restriction de public SAML contient les bonnes informations sur le locataire et sur la
        connexion (`<saml:AudienceRestriction><saml:Audience>urn:auth0:\{TENANT}:\{CONNECTION}</saml:audience>`).
      </td>
    </tr>
    <tr>
      <td>Dénomination</td>
      <td>
        L’attribut identifié par le champ `NameIdentifier` doit être connu de l’application. Si ce n’est pas le cas,
        l’identifiant doit être un autre attribut dans l’assertion (tel qu’un identifiant IdP interne pour l’utilisateur
    ...
    <tr>
      ...
      <td>
        Vérifiez que la valeur indiquée par l’élément `X509Certificate` correspond à la valeur fournie à votre
        connexion.
      </td>
    </tr>
    <tr>
...
4. Si l’attribut est manquant, vérifiez s’il a été inclus dans l’assertion. Vous pouvez le faire en décodant l’assertion SAML, ou vous pouvez activer le débogage pour la connexion.
   ...
   2. Ouvrez la liste des connexions de l’IdP **SAML** , cliquez sur ](/docs/images/ (Paramètres)**, et activez **Debug Mode (Mode débogage)\*\*.
      ...
      <Frame>
        ![Troubleshoot SAML Connections Enable Debug Mode
        screen](/docs/images/fr-ca/cdy7uua7fh8z/1F6npTHy1P0c0ygXfbndBD/a8f34635de6282df5062c2f951ee3177/2025-02-27_15-33-50.png)
      </Frame>
...
5. Si une valeur d’attribut existe dans le profil utilisateur Auth0, mais n’est pas mappée au bon attribut, vous pouvez corriger cela via la fonctionnalité de mappage de connexion.
   ...
   2. Ouvrez la liste des connexions de l’IdP **SAML** et allez ](/docs/images/ **Mappages**.
      ...
      <Frame>
        ![Troubleshoot SAML Connections Mappings Tab
        Screen](/docs/images/fr-ca/cdy7uua7fh8z/BPMjuhqE17E5vWcHukcKf/2ddef4b5c2e30abde00e2f9504799c20/2025-02-27_14-14-14.png)
      </Frame>
...
4. Si votre flux d’autorisation utilise un protocole conforme à OidentifiantC, vous pouvez capturer un suivi HAR et le visualiser
à l’aide de [l’analyseur HAR de Google](https://toolbox.googleapps.com/apps/har_analyzer/).
...
4.  Si vous utilisez un flux initié par l’IdP (par exemple, l’utilisateur commence à partir du fournisseur d’identité dans une application de portail), assurez-vous que :
...
5. Assurez-vous que l’assertion SAML contient toutes les informations supplémentaires requises par l’application et que ces
informations sont présentes dans les attributs attendus par l’application.
