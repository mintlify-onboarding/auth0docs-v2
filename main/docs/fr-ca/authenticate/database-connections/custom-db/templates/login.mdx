...
Ce script est nécessaire à la fois pour l’authentification traditionnelle et pour la [migration automatique](/docs/docs/fr-ca/manage-users/user-migration/configure-automatic-migration-from-your-database). Si la migration automatique est configurée pour la connexion, le processus de migration sera déclenché après la première connexion réussie de l’utilisateur.
...
Si l’utilisateur s’authentifie avec succès, les données de son profil doivent être renvoyées dans l’objet `profile` sous [forme normalisée](/docs/docs/fr-ca/manage-users/user-accounts/user-profiles/normalized-user-profile-schema). Outre les champs standard, vous pouvez inclure les champs `user_metadata`, `app_metadata` et `mfa_factors`.
...
L’objet de type erreur personnalisé `WrongUsernameOrPasswordError` vous permet de transmettre des données qui seront affichées dans vos [Journaux du locataire](/docs/docs/fr-ca/deploy-monitor/logs).
...
* [Créer des modèles de script](/docs/docs/fr-ca/authenticate/database-connections/custom-db/templates/create)
