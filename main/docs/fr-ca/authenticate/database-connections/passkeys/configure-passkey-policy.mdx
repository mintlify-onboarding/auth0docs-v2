...
Vous pouvez activer les clés d’identification comme méthode d’authentification et configurer votre politique de clé d’identification directement depuis le <Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
Activez et configurez la [New Universal Login Experience (Nouvelle expérience de connexion universelle)](/docs/docs/fr-ca/authenticate/login/auth0-universal-login/universal-login-vs-classic-login/universal-experience).

#### Désactiver le paramètre Custom Login Page (Page de connexion personnalisée)

Le paramètre **Customize Login Page (Personnaliser la page de connexion)** vous permet de personnaliser les pages de connexion lorsque vous utilisez un [custom domain (domaine personnalisé)](/docs/docs/fr-ca/customize/custom-domains).
...
Activez et configurez [Identifier First authentication (Authentification Identifier First)](/docs/docs/fr-ca/authenticate/login/auth0-universal-login/identifier-first).
...
Le réglage **Use my own database (Utiliser ma propre base de données)** vous permet d’utiliser une base de données externe pour stocker les utilisateurs sur une connexion [custom database (base de données personnalisée)](/docs/docs/fr-ca/authenticate/database-connections/custom-db/create-db-connection).

Les réglages **Importer des utilisateurs vers Auth0** vous permettent d’importer automatiquement des utilisateurs d’une base de données externe vers Auth0. Pour plus d’informations, veuillez consulter [Importer et exporter des utilisateurs](/docs/docs/fr-ca/manage-users/user-migration/user-migration-scenarios).
