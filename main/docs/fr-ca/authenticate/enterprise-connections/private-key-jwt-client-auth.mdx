...
You can set up Private Key JWT Client Authentication for your OIDC and Okta Workforce enterprise connections through either the <Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
Principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire">Auth0 Dashboard</Tooltip> or the <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip>.

## How it works

The OIDC Connection flow uses authenticated endpoints such as `/oauth/token` or `/oauth/par` to verify a client’s identity against the <Tooltip href="/docs/docs/fr-ca/glossary?term=authorization-server" tip="Serveur d’autorisations :
...
* An `aud` (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-6" tip="">audience</Tooltip>) identifying the OpenID provider’s <Tooltip href="/docs/docs/fr-ca/glossary?term=token-endpoint" tip="Point de terminaison de jeton
...
* A `sub` and `iss` identifying the <Tooltip href="/docs/docs/fr-ca/glossary?term=client-id" tip="ID client
...
After a user completes authentication with an upstream <Tooltip href="/docs/docs/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
To create a new OIDC connection that uses Private Key JWT Client Authentication, call the [Create a Connection](/docs/docs/fr-ca/api/management/v2/connections/post-connections) endpoint with the following `connection.options` properties set appropriately:
...
To modify an existing OIDC connection to use Private Key JWT Client Authentication, call the [Update a Connection](/docs/docs/fr-ca/api/management/v2/connections/patch-connections-by-id) endpoint with the following `connection.options` properties set appropriately:
...
* [Connectez votre application Auth0 à votre connexion Okta Workforce Enterprise](/docs/docs/fr-ca/authenticate/identity-providers/enterprise-identity-providers/okta)
* [Se connecter au Fournisseur d’identité OpenID Connect](/docs/docs/fr-ca/authenticate/identity-providers/enterprise-identity-providers/oidc)