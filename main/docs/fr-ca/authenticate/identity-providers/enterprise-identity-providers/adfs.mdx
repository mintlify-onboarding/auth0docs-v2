...
* Point de terminaison : `https://{yourDomain}/login/callback` ou `https://<YOUR CUSTOM DOMAIN>/login/callback`, si vous utilisez un [custom domain](/docs/docs/fr-ca/customize/custom-domains).
...
Si vous utilisez la fonctionnalité [domaines personnalisés](/docs/docs/fr-ca/customize/custom-domains) vous devrez remplacer la valeur `$webAppEndpoint` par `https://<YOUR CUSTOM DOMAIN>/login/callback`.
...
8. Cochez **Activer le support de la WS-Federation...**, et saisissez cette valeur dans la zone de texte :
   `https://{yourDomain}/login/callback`,ou si vous utilisez un [domaine personnalisé](/docs/docs/fr-ca/customize/custom-domains), utilisez `https://<YOUR CUSTOM DOMAIN>/login/callback`
...
3. Si le type de demande que vous recherchez n’existe pas, vous avez deux options :
   ...
   1. Entrez un [nom qualifié de namespace](/docs/docs/fr-ca/secure/tokens/json-web-tokens/create-custom-claims) pour la nouvelle demande (par exemple, `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/department`).
...
* [Commencez avec nos guides de démarrage rapide](/docs/docs/fr-ca/quickstarts)
* [Configurez votre application en utilisant notre formulaire de connexion Lock.](/docs/docs/fr-ca/libraries/lock)
* [Configurez votre application en utilisant notre bibliothèque Auth0.js et votre propre interface utilisateur](/docs/docs/fr-ca/libraries/auth0js)
* [Utilisez notre Authentication API pour vous authentifier.](/docs/docs/fr-ca/api/authentication)