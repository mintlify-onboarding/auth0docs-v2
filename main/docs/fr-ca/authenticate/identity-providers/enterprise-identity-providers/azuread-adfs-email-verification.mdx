...
Lorsque la propriété est définie sur **Always set (Toujours définir)** `email_verified` **to (sur)** `false`, les utilisateurs obtiendront `email_verified` défini sur `false` la prochaine fois qu’ils se connecteront, à moins que le paramètre [synchroniser les attributs du profil utilisateur à chaque connexion](/docs/docs/fr-ca/manage-users/user-accounts/user-profiles/configure-connection-sync-with-auth0) soit désactivé.
...
Ce nouvel écran affiche l’expérience de connexion universelle, même si vous utilisez la connexion classique. Pour savoir comment le personnaliser, consultez [Personnaliser les pages de connexion universelle](/docs/docs/fr-ca/customize/login-pages/universal-login/customize-templates).

Pour savoir comment personnaliser le courriel envoyé aux utilisateurs, veuillez consulter [Courriels de vérification avec Auth0.](/docs/docs/fr-ca/manage-users/user-accounts/verify-emails)
