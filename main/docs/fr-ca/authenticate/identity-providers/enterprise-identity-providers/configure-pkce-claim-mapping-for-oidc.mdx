...
Si votre fournisseur d’identité OIDC (<Tooltip href="/docs/docs/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
Vous pouvez consulter la configuration PKCE pour une connexion spécifique via le <Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
Les demandes mappées ne sont pas automatiquement ajoutées à un jeton d’ID d’Auth0. Pour ajouter des demandes à un jeton d’ID, voir [Créer des demandes personnalisées](/docs/docs/fr-ca/secure/tokens/json-web-tokens/create-custom-claims#create-custom-claims).
...
Pour une liste complète des demandes restreintes, consulter [Créer des demandes personnalisées](/docs/docs/fr-ca/secure/tokens/json-web-tokens/create-custom-claims).
