...
The Sign in with Google flow leverages the <Tooltip href="/docs/docs/fr-ca/glossary?term=oath2" tip="OAuth 2.0
...
After creating your credentials in the Google Cloud Console, you can enable Sign in with Google using the Credential Manager for Android. To do so, update your application through either the <Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
4. **For new applications**: On the Device Settings tab, complete the fields in the **Android** section, including **App Package Name**. For more information, review [Enable Android App Links Support](/docs/docs/fr-ca/get-started/applications/enable-android-app-links-support).
5. Select **Save Changes**.

</Tab><Tab title="Management API">

To update your application through the Management API, call the [Update a client](/docs/docs/fr-ca/api/management/v2/clients/patch-clients-by-id) endpoint as follows:
...
While the current Google implementation does not offer clear guidance for enterprise federation and additional <Tooltip href="/docs/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
To do so, decode the Google <Tooltip href="/docs/docs/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
Format standard de jeton d’ID (et souvent de jeton d’accès) utilisé pour représenter en toute sécurité des demandes entre deux parties." cta="Voir le glossaire">JWT</Tooltip> token and pass the `email` provided in the <Tooltip href="/docs/docs/fr-ca/glossary?term=id-token" tip="Jeton d’ID
