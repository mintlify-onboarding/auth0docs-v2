...
Le [protocole OpenID Connect](/docs/docs/fr-ca/authenticate/protocols/openid-connect-protocol) prend en charge un paramètre `prompt=none` sur la demande d’authentification qui permet aux applications d’indiquer que le serveur d’autorisation ne doit afficher aucune interaction utilisateur (telle que l’authentification, le consentement ou la <Tooltip href="/docs/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
Toutes les [règles](/docs/docs/fr-ca/customize/rules) applicables seront exécutées dans le cadre du processus d’authentification silencieuse.
...
Si l’utilisateur n’est pas connecté via l’authentification unique (<Tooltip href="/docs/docs/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
<td>L’utilisateur n’était pas connecté à Auth0, donc l’authentification silencieuse n’est pas possible. Cette erreur peut se produire en fonction de la configuration des paramètres <strong>Log In Session Management (Gestion de session de connexion)</strong> au niveau du locataire; en particulier, elle peut se produire après la période définie dans le paramètre <strong>Require log in after (Exiger la connexion après)</strong>. Consultez <a href="/docs/docs/fr-ca/dashboard/guides/tenants/configure-session-Lifetime-settings">Configuration des paramètres de durée de vie des sessions</a> pour plus de détails.</td>
</tr>
<tr>
...
<td>L’utilisateur était connecté à Auth0 et a autorisé l’application, mais doit être redirigé ailleurs avant que l’authentification puisse être terminée; par exemple, lors de l’utilisation d’une <a href="/docs/docs/fr-ca/rules/redirect">règle de redirection</a>.</td>
</tr>
</tbody>
</table>
...
Dans le cas d’un [Flux implicite](/docs/docs/fr-ca/get-started/authentication-and-authorization-flow/implicit-flow-with-form-post), le paramètre `expires_in` est renvoyé par Auth0 sous forme de paramètre de hachage suite à une authentification réussie. Dans le [Flux de code d’autorisation avec PKCE](/docs/docs/fr-ca/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce), il est renvoyé au serveur dorsal lors de l’exécution de l’échange de code d’autorisation.
...
Pour en savoir plus, veuillez consulter [Modifier la fréquence des demandes d’authentification](/docs/docs/fr-ca/secure/multi-factor-authentication/customize-mfa).

## En savoir plus

* [Rotation des jetons d’actualisation](/docs/docs/fr-ca/secure/tokens/refresh-tokens/refresh-token-rotation)
* [Configurer la rotation des jetons d’actualisation](/docs/docs/fr-ca/secure/tokens/refresh-tokens/configure-refresh-token-rotation)
* [Dépannage des erreurs de jeton non valide](/docs/docs/fr-ca/troubleshoot/basic-issues/invalid-token-errors)
