...
4. Auth0 envoie un jeton d’ID avec `sid` à l’application A. Pour en savoir plus, consultez [Structure du jeton d’ID](/docs/docs/fr-ca/secure/tokens/id-tokens/id-token-structure).
...
Votre application doit être en mesure d’analyser et de valider les <Tooltip href="/docs/docs/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
Format standard de jeton d’ID (et souvent de jeton d’accès) utilisé pour représenter en toute sécurité des demandes entre deux parties." cta="Voir le glossaire">JWT</Tooltip> pour les utiliser comme jetons de déconnexion avec Auth0. Pour en savoir plus, consultez [Valider les jetons Web JSON](/docs/docs/fr-ca/secure/tokens/json-web-tokens/validate-json-web-tokens).
...
* Les applications doivent vérifier tout jeton reçu conformément aux [pratiques exemplaires de validation des JWT](/docs/docs/fr-ca/secure/tokens/json-web-tokens/validate-json-web-tokens).
...
* Il est recommandé que les applications acceptent uniquement les demandes provenant de la liste publiée des [adresses IP sortantes](/docs/docs/fr-ca/secure/security-guidance/data-security/allowlist).
...
* [Configuration de déconnexion du canal d’appui OIDC](/docs/docs/fr-ca/authenticate/login/logout/back-channel-logout/configure-back-channel-logout)
