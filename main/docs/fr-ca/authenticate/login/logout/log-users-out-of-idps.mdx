...
Pour ajouter la déconnexion fédérée au [point de terminaison de déconnexion alternatif](/docs/docs/fr-ca/api/authentication#logout), ajoutez le paramètre de chaîne de requête `federated` à l’URL de déconnexion :
...
* Si vous travaillez avec des fournisseurs d’identité sociale tels que Google ou Facebook, vous devez définir votre <Tooltip href="/docs/docs/fr-ca/glossary?term=client-id" tip="ID client
...
* Si vous êtes un utilisateur Auth0 Enterprise, vous avez généralement activé le <Tooltip href="/docs/docs/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
