...
Activez le Workforce Identity Cloud d’Okta en tant que fournisseur d’identité en suivant les instructions sur la [page d’aide Fournisseurs d’identité](/docs/docs/fr-ca/authenticate/identity-providers/enterprise-identity-providers/okta).
...
Cette section décrit comment configurer une intégration <Tooltip href="/docs/docs/fr-ca/glossary?term=openid" tip="OpenID
...
Pour plus d’informations sur la configuration d’une intégration <Tooltip href="/docs/docs/fr-ca/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language (SAML)
Protocole normalisé permettant à deux parties d’échanger des informations d’authentification sans mot de passe." cta="Voir le glossaire">SAML</Tooltip> et SCIM avec le nuage d’identité Okta Workforce, consultez plutôt [SCIM entrant pour les connexions SAML d’Okta Workforce](/docs/docs/fr-ca/authenticate/protocols/scim/inbound-scim-for-okta-workforce-saml-connections).
...
2. Désactivez **Synchroniser les attributs du profil utilisateur à chaque connexion** sauf si vous souhaitez [sync additional attributes at login (synchroniser des attributs supplémentaires à la connexion)](/docs/docs/fr-ca/authenticate/protocols/scim/configure-inbound-scim).
