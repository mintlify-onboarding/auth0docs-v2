...
Native to Web SSSO does not change standard Auth0 [Single Sign-On](/docs/docs/fr-ca/authenticate/single-sign-on) authentication.

</Callout>

**Specific Native to Web <Tooltip href="/docs/docs/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
A [`session_transfer_token`](/docs/docs/fr-ca/authenticate/single-sign-on/native-to-web/configure-implement-native-to-web#create-and-manage-session-transfer-tokens) is used to initiate a secure session in a WebView or browser to securely authenticate the user without being prompted to login. These web sessions may also issue their own <Tooltip href="/docs/docs/fr-ca/glossary?term=refresh-token" tip="Jeton d’actualisation
