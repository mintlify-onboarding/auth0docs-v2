...
* Utilisez les [métadonnées d’application](/docs/docs/fr-ca/get-started/applications/configure-application-metadata) pour filtrer les applications spécifiques afin de déterminer si une Action doit être exécutée.
* Veiller à ce que les Actions qui fournissent une vérification ou déclenchent une <Tooltip href="/docs/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
* Utilisez `event.request.hostname` pour le domaine utilisé dans les appels d’Authentication API; il peut s’agir du domaine de locataire Auth0 par défaut ou d’un [domaine personnalisé](/docs/docs/fr-ca/customize/custom-domains).
...
* Différentes connexions fournissent des données de profil utilisateur différentes; le seul [champ du profil utilisateur](/docs/docs/fr-ca/manage-users/user-accounts/user-profiles/user-profile-structure) garanti est le `user_id`.
