...
* Chaque exécution d’un flux doit se terminer en 20 secondes ou moins, sinon le traitement se terminera par une erreur. Limiter les processus de longue durée, comme les requêtes HTTP sortantes sans délai d’attente, est nécessaire pour respecter ce délai. Une Action qui [redirige les utilisateurs vers une page externe](/docs/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions) a un délai d’attente distinct avant et après la redirection.
* Les appels effectués vers <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip> Auth0 et les mises à jour des métadonnées utilisateur sont [limités](/docs/docs/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy/management-api-endpoint-rate-limits).
...
* Les appels HTTP vers des services externes proviennent de [ces adresses IP](/docs/docs/fr-ca/secure/security-guidance/data-security/allowlist).
...
* Un nouvel objet `event.request` est émis à chaque fois qu’un flux d’Action est suspendu, puis repris par la suite (par exemple, en raison d’une redirection ou d’un défi <Tooltip href="/docs/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
* Un maximum de 100 attributs <Tooltip href="/docs/docs/fr-ca/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language (SAML)
...
* La récupération des jetons <Tooltip href="/docs/docs/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
