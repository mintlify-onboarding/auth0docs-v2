...
Avant de commencer la migration, consultez la page [Migration des Règles vers les Actions](/docs/docs/fr-ca/customize/actions/migrate/migrate-from-rules-to-actions). Ce guide de migration est une excellente ressource qui explique les différences entre les règles et les actions; il s’agit d’un outil utile dans le parcours de migration.
...
Pour de plus amples renseignements sur la fonction `api.rules.wasExecuted()`, référez-vous à [Déclencheurs d’actions : post-connexion – Objet API](/docs/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object).
...
Les actions peuvent être testées directement dans l’éditeur, car il s’agit d’une partie essentielle du parcours de migration. Pour de plus amples renseignements, voir [Test des actions](/docs/docs/fr-ca/customize/actions/test-actions).
...
Pour de plus amples renseignements sur l’ajout de votre action au flux, référez-vous à la section [Écrire votre première action](/docs/docs/fr-ca/customize/actions/write-your-first-action).
