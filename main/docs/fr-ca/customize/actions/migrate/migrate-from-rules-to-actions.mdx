...
Les Actions Post-connexion s’exécutent après les règles existantes. Vous pouvez donc convertir les règles individuellement dans le Dashboard ou en une seule fois à l’aide de <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
...
* Envisagez de [personnaliser votre page de connexion](/docs/docs/fr-ca/customize) temporairement pour interrompre les ouvertures de session si la conversion risque d’entraîner des ouvertures de session non valides ou des lacunes dans la protection.
* Envisagez d’utiliser l’outil [Auth0 Deploy CLI](/docs/docs/fr-ca/deploy-monitor/deploy-cli-tool) afin de rédiger des scripts, de tester et de mettre en œuvre rapidement la migration en une seule fois ou de manière itérative.
...
* Les Actions ne sont pas fournies avec [un jeton d’accès pour Management API](/docs/docs/fr-ca/customize/rules/use-management-api) ou [l’accès à l’objet `auth0` global](/docs/docs/fr-ca/rules-best-practices/rules-environment-best-practices) comme pour les Règles. Pour savoir comment les appels à Management API peuvent encore être effectués, veuillez consulter la section [Conversion du code](#convert-code).

Pour obtenir la liste complète des limitations, veuillez consulter la section [Limitations des actions](/docs/docs/fr-ca/customize/actions/limitations).
...
* Avant de mettre en ligne, vous devez soigneusement [tester vos nouvelles Actions](/docs/docs/fr-ca/customize/actions/test-actions) dans un [environnement d’essai ou de test](/docs/docs/fr-ca/get-started/auth0-overview/create-tenants/set-up-multiple-environments).
...
Pour les Règles, les données relatives à l’utilisateur qui se connecte sont stockées dans [l’objet `user`](/docs/docs/fr-ca/customize/rules/user-object-in-rules). Pour les Actions, ces données se trouvent dans la propriété `user` de [l’objet `event`](/docs/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object). La majorité des propriétés existantes sont accessibles dans ce nouvel emplacement.
...
Pour les Règles, les données relatives à la session de connexion en cours sont stockées dans [l’objet `context`](/docs/docs/fr-ca/customize/rules/context-object). Pour les Actions, ces données ont été remodelées et déplacées vers [l’objet `event`](/docs/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object). De nombreuses propriétés ont été transférées telles quelles, tandis que d’autres ont été combinées pour plus de clarté.
...
2. Ajoutez un secret pour chaque valeur à laquelle vous devez accéder à l’intérieur de l’action. Pour en savoir plus, consultez la section **Ajouter un secret** dans [Programmer votre première Action](/docs/docs/fr-ca/customize/actions/write-your-first-action).
...
Les Règles et les Actions permettent toutes deux d’ajouter des demandes personnalisées aux jetons d’ID et d’accès. Pour les Règles, il s’agit d’une propriété de l’objet `context` tandis que les Actions utilisent une méthode de [l’objet `api`](/docs/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object).
...
Pour les Règles, l’authentification multifacteur (<Tooltip href="/docs/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
Processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, tel qu’un code par SMS." cta="Voir le glossaire">MFA</Tooltip>) peut être déclenchée en modifiant la propriété `multifacteur` de [l’objet `context`](/docs/docs/fr-ca/customize/rules/context-object). Dans les Actions, cela se fait avec une [méthode sur l’objet `api`](/docs/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object).
...
De manière générale, nous vous déconseillons d’appeler Management API à partir d’un chemin critique à fort trafic comme les Règles ou les Actions. Toutes les demandes adressées aux API Auth0 sont soumises à la [limite anti-attaques](/docs/docs/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy), y compris les appels émis par les points d’extension. Le fait d’appeler une API pour chaque connexion pourrait facilement entraîner des échecs de connexion pendant les périodes de forte affluence.
...
1. [Enregistrer une application de communication entre machines et autorisez-la à utiliser Management API](/docs/docs/fr-ca/get-started/auth0-overview/create-applications/machine-to-machine-apps).
2. Enregistrer le **Client ID (ID Client)** et **Client Secret (Secret du client)** dans l’Action.
3. [Obtenir un jeton d’accès pour Management API](/docs/docs/fr-ca/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production).
...
Expliquer comment faire toutes les redirections correctement dans Actions dépasse la portée de ce guide. Pour plus de détails, consultez [Rediriger avec des Actions](/docs/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions).
