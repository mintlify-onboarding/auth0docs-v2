...
Ce guide vous explique comment créer et déployer votre première action à l’aide du <Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
2. Sélectionnez le [Déclencheur de connexion](/docs/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) et faites glisser `my-slack-action` dans le flux.
...
L’action s’exécute maintenant dans votre locataire pour un trafic d’utilisateurs réels. Si vous souhaitez disposer d’un environnement de test plus robuste, envisagez de [mettre en place plusieurs environnements Auth0](/docs/docs/fr-ca/get-started/auth0-overview/create-tenants/set-up-multiple-environments).
...
Maintenant que vous avez écrit votre première action, consultez [Explorer les flux et les déclencheurs](/docs/docs/fr-ca/customize/actions/explore-triggers) pour apprendre à effectuer certaines tâches courantes à l’aide d’actions.