...
* Your tenant is on an Enterprise plan ([Public Cloud or Private Cloud deployments](/docs/docs/fr-ca/deploy-monitor/deployment-options)). For more information, see
...
Auth0 will enable the MCD feature on tenants requested by EA participants. You can then add MCD to a tenant with the <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip> or in the <Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
3. In the configuration form, provide the following information:
   ...
   * **Certificate type:** Choose [**Auth0-managed certificates**](/docs/docs/fr-ca/customize/custom-domains/auth0-managed-certificates) or [**Self-managed certificates**](/docs/docs/fr-ca/customize/custom-domains/self-managed-certificates).
...
To create a custom domain with the Management API, you need a machine-to-machine (M2M) application that is  authorized to use the  Management API. For more information, review. For more information, review [Management API Access Tokens](/docs/docs/fr-ca/secure/tokens/access-tokens/management-api-access-tokens).
...
7. Review [Get Management API Access Tokens](/docs/docs/fr-ca/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) to retrieve and store your access token.
...
export const codeExample2 = `var client = new RestClient("https://{yourDomain}/api/v2/custom-domains");
...
export const codeExample4 = `HttpResponse<String> response = Unirest.post("https://{yourDomain}/api/v2/custom-domains")
  .header("authorization", "Bearer {yourMgmtApiAccessToken}")
  .header("content-type", "application/json")
  .header("accept", "application/json")
  .body("{ \n \"domain\": \"your.example-custom-domain.com\", \n \"type\": \"auth0_managed_certs\", \n \"tls_policy\": \"recommended\", \n \"custom_client_ip_header\": \"true-client-ip\", \n \"domain_metadata\": {\n    \"environment\": \"development\"} \n}")
...
After your custom domain has been verified by Auth0, you can use it immediately to configure Auth0 features for your users. For more information, read [Configure Features to Use Custom Domains](/docs/docs/fr-ca/customize/custom-domains/configure-features-to-use-custom-domains).

### Metadata for enhanced management

You can provision up to 10 metadata fields per custom domain for easier organization and future customization. In upcoming releases, these metadata fields will enable advanced customization of email templates, <Tooltip href="/docs/docs/fr-ca/glossary?term=universal-login" tip="Connexion universelle
...
Auth0 [Actions](/docs/docs/fr-ca/customize/actions) allows you to create custom logic handling of your different transactions based on the custom domain.

For example, you could create an Action that directs a user to an associated [Organization](/docs/docs/fr-ca/manage-users/organizations), or enforce a specific [access control policy](/docs/docs/fr-ca/customize/actions/use-cases#api-authorization).
