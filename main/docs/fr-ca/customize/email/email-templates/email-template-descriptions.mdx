...
Pour certaines erreurs, l’utilisateur peut être envoyé vers la page [Erreur générique](/docs/docs/fr-ca/authenticate/login/auth0-universal-login/error-pages) au lieu de l’URL de redirection.

</Callout>

Vous pouvez éviter l’inclusion des adresses courriel dans les liens de redirection en mettant à jour les modèles de courriel via <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
...
* Si l’utilisateur s’est connecté via une [Organization](/docs/docs/fr-ca/manage-users/organizations/organizations-overview) :
...
* `url` : URL que l’utilisateur choisira pour modifier son mot de passe. Vous l’utiliserez dans le champ **Message** pour créer un lien que l’utilisateur peut suivre, comme dans cet exemple :
  `<a href="/docs/docs/fr-ca/{{ url }}">Click here to change your password</a>`
* Si l’utilisateur s’est connecté via une [Organization](/docs/docs/fr-ca/manage-users/organizations/organizations-overview) :
...
`<a href="/docs/docs/fr-ca/{{ url }}">Click here to unblock your account</a>`
...
Ce courriel sera généré lors de l’envoi d’une invitation d’inscription à <Tooltip href="/docs/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
`<a href="/docs/docs/fr-ca/{{ link }}">Enroll your MFA device</a>`
...
Si un utilisateur est invité à devenir membre d’une [Organization](/docs/docs/fr-ca/manage-users/organizations/organizations-overview), il recevra un courriel **User Invitation (Invitation de l’utilisateur)** contenant un lien URL. Lorsque l’utilisateur clique sur le lien, il sera redirigé vers votre route de connexion par défaut. Pour en savoir plus, consultez [Configurer les routes de connexion par défaut](/docs/docs/fr-ca/authenticate/login/auth0-universal-login/configure-default-login-routes).
...
Si l’utilisateur n’accepte pas une invitation avant qu’elle n’expire, l’invitation sera marquée comme expirée et éventuellement supprimée d’[Auth0 Dashboard](/docs/docs/fr-ca/get-started/auth0-overview/dashboard).
...
Pour localiser les courriels, vous pouvez définir la variable `locale` à envoyer dans `user_metadata` lors de la création d’une invitation. Pour en savoir plus, consultez [Personnaliser les modèles de courriel : Modèles de courriels multilingues](/docs/docs/fr-ca/customize/email/email-templates).

### Redirige les résultats pour les modèles de courriel d’invitation des utilisateurs

Vous devez [configurer une route de connexion par défaut](/docs/docs/fr-ca/authenticate/login/auth0-universal-login/configure-default-login-routes) vers laquelle les utilisateurs seront envoyés après avoir sélectionné le lien pour accepter l’invitation. Lors de la redirection, Auth0 inclura les paramètres suivants :
...
{% else %}
  ...
  {% if send == 'link' or send == 'link_ios' or send == 'link_android' %}
    ...
    <a href="/docs/docs/fr-ca/{{ link }}">Sign in to {{ application.name }}</a>
    {% elsif send == 'code' %}
