...
* [Personnaliser un modèle de courriel](/docs/docs/fr-ca/customize/email/email-templates) et l’utiliser pour [envoyer un courriel de modification du mot de passe](/docs/docs/fr-ca/customize/email/manage-email-flow).
* Créer un ticket de modification de mot de passe.

Vous pouvez autoriser un utilisateur à accéder à un compte existant que vous avez créé en son nom. Ensuite, envoyez à l’utilisateur un lien unique pour qu’il puisse définir son mot de passe. Vous générez le lien unique en créant un ticket de modification de mot de passe dans lequel votre application d’invitation appelle le point de terminaison `/password-change`[Management API](/docs/docs/fr-ca/api/management/v2/#!/Tickets/post_password_change). Vous aurez besoin de :

* [Créer un utilisateur de la base de données Auth0](/docs/docs/fr-ca/authenticate/database-connections) avec le paramètre `user.email_verified` défini sur `false`. Vous pouvez utiliser le point de terminaison [Créer un utilisateur](/docs/docs/fr-ca/api/management/v2/#!/Users/post_users).
* Avoir accès à un [service de courriel externe et le configurer](/docs/docs/fr-ca/customize/email/smtp-email-providers).
* [Obtenir un jeton d’accès à Management API](/docs/docs/fr-ca/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-testing).
...
Pour en savoir plus, consultez [Configuration des itinéraires de connexion par défaut](/docs/docs/fr-ca/authenticate/login/auth0-universal-login/configure-default-login-routes).
