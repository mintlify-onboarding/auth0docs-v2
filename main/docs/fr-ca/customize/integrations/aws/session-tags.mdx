...
* [ConfigureR AWS pour la SSO](/docs/docs/fr-ca/customize/integrations/aws/configure-amazon-web-services-for-sso)
...
Create an IAM role using the AWS SAML <Tooltip href="/docs/docs/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
Pour mapper le rôle AWS et les étiquettes à un utilisateur, vous devez [créer une règle](/docs/docs/fr-ca/customize/rules/create-rules) dans Auth0. Ces valeurs seront ensuite transmises aux assertions SAML dans le jeton. Pour l’exemple :
...
To log in, you will need the <Tooltip href="/docs/docs/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
