...
Connexion par Auth0 peut être configuré à l’aide de l’assistant de configuration dans le plugiciel ou manuellement pour plus de contrôle sur le processus. Pour en savoir plus, veuillez consulter [Installer la connexion par Auth0](/docs/docs/fr-ca/customize/integrations/cms/wordpress-plugin/install-login-by-auth0). Les instructions ci-dessous peuvent également être utilisées si l’assistant de configuration n’a pas été complété ou dans le cadre de la résolution des problèmes de connexion.
...
Votre locataire Auth0 doit être configuré pour accepter les demandes de connexion à partir de votre site WordPress et les identités d’utilisateur source d’au moins une connexion, qu’il s’agisse d’une base de données Auth0, d’une connexion via un réseau social ou d’un répertoire des entreprises. Pour en savoir plus sur les locataires, veuillez consulter [Créer des locataires](/docs/docs/fr-ca/get-started/auth0-overview/create-tenants). Pour en savoir plus sur les connexions, veuillez consulter [Fournisseurs d’identité](/docs/docs/fr-ca/authenticate/identity-providers).
...
L’URL de rappel **ne doit pas** être mise en cache, sinon une erreur « État non valide » risque de se produire lors de la connexion. Pour en savoir plus, consultez [Dépannage des erreurs d’état non valide des modules d’extension WordPress](/docs/docs/fr-ca/customize/integrations/cms/wordpress-plugin/troubleshoot-wordpress-plugin-invalid-state-errors).
...
Pour que votre site WordPress puisse effectuer certaines actions au nom de votre locataire Auth0, vous devrez autoriser l’application créée ci-dessus à accéder à <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
...
Pour savoir comment activer et configurer cette méthode de connexion, veuillez consulter [Fournisseurs d’identité sociale](/docs/docs/fr-ca/authenticate/identity-providers/social-identity-providers).

### Configuration de la connexion d’entreprise

Pour savoir comment activer et configurer cette méthode de connexion, veuillez consulter [Enterprise Identity Providers](/docs/docs/fr-ca/authenticate/identity-providers/enterprise-identity-providers) (Fournisseurs d’identité de l’entreprise).
...
* **Domaine personnalisé** : Le domaine personnalisé pour votre locataire, si configuré. Pour en savoir plus, veuillez consulter [Domaines personnalisés](/docs/docs/fr-ca/customize/custom-domains). Le nom de l’option est `custom_domain`.
...
* **Algorithme de signature <Tooltip href="/docs/docs/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
Format standard de jeton d’ID (et souvent de jeton d’accès) utilisé pour représenter en toute sécurité des demandes entre deux parties." cta="Voir le glossaire">JWT</Tooltip>** L’algorithme utilisé pour signer les jetons à partir de l’onglet Paramètres avancés de l’application, <Tooltip href="/docs/docs/fr-ca/glossary?term=oath2" tip="OAuth 2.0
...
* **Page de connexion universelle** : Redirige la page `wp-login.php` vers la page de connexion universelle pour l’authentification unique <Tooltip href="/docs/docs/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
Les options ici n’affectent pas la page de connexion universelle (pour en savoir plus sur les options de personnalisation, veuillez consulter [Connexion universelle Auth0](/docs/docs/fr-ca/authenticate/login/auth0-universal-login).
...
* **Couleur primaire** : Pour en savoir plus sur ce paramètre, veuillez consulter [Options de configuration Lock](/docs/docs/fr-ca/libraries/lock/lock-configuration). Le nom de l’option est `primary_color`.
...
* [Dépannage du plugiciel WordPress Login by Auth0](/docs/docs/fr-ca/customize/integrations/cms/wordpress-plugin/troubleshoot-login-by-auth0)
