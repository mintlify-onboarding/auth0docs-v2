...
La façon la plus simple de planifier la migration des utilisateurs est d’utiliser l’assistant de configuration lorsque le plugiciel est installé pour la première fois. Pour en savoir plus sur le processus, lisez [Installer Login by Auth0](/docs/docs/fr-ca/customize/integrations/cms/wordpress-plugin/install-login-by-auth0).
...
1. [Créez et configurez correctement une application](/docs/docs/fr-ca/customize/integrations/cms/wordpress-plugin/configure-login-by-auth0), et créez et activez une connexion de base de données vide pour l’application. Il peut s’agir des mêmes que celles créées dans le processus standard de l’assistant d’installation, ou vous pouvez les créer à partir de zéro. Pour en savoir plus sur l’assistant d’installation, lisez [Installer la connexion par Auth0](/docs/docs/fr-ca/customize/integrations/cms/wordpress-plugin/install-login-by-auth0).
...
* Si vous avez des données dans Auth0 qui doivent être conservées, vous pouvez utiliser [notre extension Importation/Exportation des utilisateurs](/docs/docs/fr-ca/manage-users/user-migration/user-import-export-extension) pour ajuster les données de l’utilisateur.
  ...
  . 3. Modifiez tous les identifiants des utilisateurs pour ajouter l’espace de noms utilisé lors de la création de la nouvelle connexion. Les identifiants d’utilisateurs devraient passer de `auth0|123` à `auth0|Votre-nom-de-site-WP|123`. Ajustez tous les autres champs dont vous avez besoin pour suivre le schéma d’importation. Pour en savoir plus, lisez [Importation d’utilisateurs en lots, schéma de base de données et exemples](/docs/docs/fr-ca/manage-users/user-migration/bulk-user-import-database-schema-and-examples).
