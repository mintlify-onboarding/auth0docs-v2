...
You can use [Auth0 Rules](/docs/fr-ca/customize/rules) to redirect users before an authentication transaction is complete. This lets you implement custom authentication flows that require additional user interaction beyond the standard login form. Redirect rules are commonly used to do custom <Tooltip href="/docs/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
If you're using a <Tooltip href="/docs/docs/fr-ca/glossary?term=custom-domain" tip="Domaine personnalisé
...
* any changes to the user object are made during the redirect, prior to calling the `/continue` endpoint. For example, updates through the Auth0 <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
...
Passing information back and forth in the front channel opens up surface area for <Tooltip href="/docs/docs/fr-ca/glossary?term=bad-actors" tip="Acteurs menaçants
...
The application is going to redirect the user back to the Auth0 tenant, so any data related to the user can be gathered through the <Tooltip href="/docs/docs/fr-ca/glossary?term=id-token" tip="Jeton d’ID
...
* For <Tooltip href="/docs/docs/fr-ca/glossary?term=refresh-token" tip="Jeton d’actualisation
...
* For <Tooltip href="/docs/docs/fr-ca/glossary?term=resource-owner" tip="Propriétaire de ressources
