...
Ce document traite des permissions incluses dans le protocole d’authentification <Tooltip href="/docs/docs/fr-ca/glossary?term=openid" tip="OpenID
...
<tr>
<td>`openid`</td>
<td>(obligatoire) Renvoie la demande `sub`, qui identifie de manière unique l’utilisateur. Dans un jeton d’ID, les demandes `iss`, `aud`, `exp`, `iat`, et `at_hash` seront également présentes. Pour en savoir plus sur les demandes du jeton d’ID, veuillez consulter <a href="/docs/docs/fr-ca/tokens/id-tokens/id-token-structure">Structure du jeton d’ID</a>.</td>
</tr>
<tr>
<td>`profile`</td>
