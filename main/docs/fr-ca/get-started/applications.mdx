...
Vous enregistrez les applications dans <AuthLink href="https://manage.auth0.com/#/applications/{yourClientId}/settings">Dashboard > Applications > Applications</AuthLink>. Outre la configuration d’applications dans le Dashboard, vous pouvez aussi en configurer par programmation, comme décrit dans la spécification [Enregistrement dynamique du client OpenID Connect (OIDC) 1.0](https://openid.net/specs/openid-connect-registration-1_0.html) .
