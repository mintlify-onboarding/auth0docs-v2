...
Pour les applications destinées aux clients, <Tooltip href="/docs/docs/fr-ca/glossary?term=openid" tip="OpenID
...
Avez-vous ou prévoyez-vous d’avoir plus d’une application dans votre système? Si la réponse à cette question est oui, il vous faut une expérience de connexion centralisée. Pour obtenir une expérience d’authentification unique (<Tooltip href="/docs/docs/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
* alors autorisez Auth0 à rediriger les utilisateurs vers votre ancien système SSO pour qu’ils se connectent. Pour ce faire, vous devez configurer votre ancien système en tant que fournisseur d’identité (<Tooltip href="/docs/docs/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
