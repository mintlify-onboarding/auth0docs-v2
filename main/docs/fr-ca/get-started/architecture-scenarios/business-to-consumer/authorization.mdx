...
Les[permissions OAuth2](/docs/fr-ca/get-started/apis/scopes/api-scopes) sont généralement utilisées comme le mécanisme par lequel une API peut déterminer les actions susceptibles d’être effectuées au nom d’un utilisateur. Les permissions peuvent être ajoutées pour chaque API afin de [définir des permissions d’accès spécifiques](/docs/fr-ca/get-started/apis/add-api-permissions) dans le <Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
Principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire">Auth0 Dashboard</Tooltip> ou via <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
...
Il existe de nombreux scénarios qui nécessitent qu’une application sans session utilisateur interactive obtienne un jeton d’accès afin d’appeler une API. Dans de tels scénarios, vous devez authentifier le client au lieu de l’utilisateur, et <Tooltip href="/docs/docs/fr-ca/glossary?term=oath2" tip="OAuth 2.0
