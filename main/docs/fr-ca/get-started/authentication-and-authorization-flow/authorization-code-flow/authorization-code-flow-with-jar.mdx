...
Pour générer une requête JAR, vous devez d’abord créer un [jeton Web JSON (JWT)](/docs/fr-ca/secure/tokens/json-web-tokens). Utilisez la [bibliothèque Auth0 JWT](https://jwt.io/libraries) pour vous aider à générer un <Tooltip href="/docs/docs/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
...
* `response_type`: indique à Auth0 le flux <Tooltip href="/docs/docs/fr-ca/glossary?term=oath2" tip="OAuth 2.0
