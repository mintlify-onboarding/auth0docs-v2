...
To start using the new theme, set up the Activity in the `AndroidManifest.xml` file with the `android:theme` attribute. Depending on if you are using Classic Lock or <Tooltip href="/docs/docs/fr-ca/glossary?term=passwordless" tip="Sans mot de passe
