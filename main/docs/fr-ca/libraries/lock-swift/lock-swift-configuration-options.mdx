...
Permission utilisée pour l’authentification. La valeur par défaut est `openid`. Elle retournera non seulement le **Jeton d’accès**, mais aussi un **Jeton d’ID** qui est un jeton Web JSON (<Tooltip href="/docs/docs/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
