...
Prenons l’exemple d’un site web conforme à l’OIDC (<Tooltip href="/docs/docs/fr-ca/glossary?term=openid" tip="OpenID
...
Dans ce scénario, trois sessions sont créées : la **session locale** (storezero.io), la **session du serveur d’autorisation** (storezero.auth0.com) et une **session du fournisseur d’identité (<Tooltip href="/docs/docs/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
Service de stockage et de gestion des identités numériques." cta="Voir le glossaire">IdP</Tooltip>)** (facebook.com).

La session de l’IdP sur le serveur de Facebook authentifie l’utilisateur et fournit une expérience <Tooltip href="/docs/docs/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
