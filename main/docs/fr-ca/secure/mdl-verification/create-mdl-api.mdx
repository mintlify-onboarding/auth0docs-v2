...
export const codeExample1 = `   curl --request POST \
   ...
   --data '{"name"}'
`;
export const codeExample2 = `   var client = new RestClient("https://{yourDomain}/api/v2/resource-servers/post-resource-servers");
...
`;
export const codeExample4 = `   HttpResponse response = Unirest.post("https://{yourDomain}/api/v2/resource-servers/post-resource-servers")
   .header("content-type", "application/json")
   .body("{\"name\"}")
   .asString();
`;
