...
Pour configurer Duo Security, vous devez transmettre vos informations d’identification Duo à votre application dans l’<Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
Pour activer Duo dans une Action, passez `duo` comme le paramètre `provider` lorsque vous activez l’authentification multifacteur (<Tooltip href="/docs/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
