...
[Sender constraining](/docs/fr-ca/secure/sender-constraining) is an <Tooltip href="/docs/docs/fr-ca/glossary?term=oath2" tip="OAuth 2.0
Cadre d’applications d’autorisation qui définit les protocoles d’autorisation et les flux de production." cta="Voir le glossaire">OAuth 2.0</Tooltip> security mechanism that cryptographically binds access and <Tooltip href="/docs/docs/fr-ca/glossary?term=refresh-token" tip="Jeton d’actualisation
Jeton utilisé pour obtenir un jeton d’accès renouvelé sans obliger les utilisateurs à se connecter à nouveau." cta="Voir le glossaire">refresh tokens</Tooltip> to the client application that requested them. This ensures that only the legitimate client that obtained the <Tooltip href="/docs/docs/fr-ca/glossary?term=access-token" tip="Jeton d’accès
...
* [Configure sender constraining](/docs/fr-ca/secure/sender-constraining/configure-sender-constraining) for your client application and <Tooltip href="/docs/docs/fr-ca/glossary?term=resource-server" tip="Serveur de ressources
...
* Ensure your client application is a <Tooltip href="/docs/docs/fr-ca/glossary?term=confidential-client" tip="Client confidentiel
...
* Before requesting an access token, your client application initiates a TLS handshake with the Auth0 <Tooltip href="/docs/docs/fr-ca/glossary?term=authorization-server" tip="Serveur d’autorisations :
...
* The token request does not include any special DPoP headers or additional proof <Tooltip href="/docs/docs/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
...
* **Confidential clients only:** mTLS sender constraining is designed for and supported only by confidential clients, such as backend services, that can securely manage client certificates and establish mTLS connections. <Tooltip href="/docs/docs/fr-ca/glossary?term=public-client" tip="Client public
