...
<Tooltip href="/docs/docs/fr-ca/glossary?term=id-token" tip="Jeton d’ID
...
Beyond what is required for <Tooltip href="/docs/docs/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
Format standard de jeton d’ID (et souvent de jeton d’accès) utilisé pour représenter en toute sécurité des demandes entre deux parties." cta="Voir le glossaire">JWT</Tooltip>, ID tokens also contain claims asserted about the authenticated user, which are pre-defined by the <Tooltip href="/docs/docs/fr-ca/glossary?term=openid" tip="OpenID
