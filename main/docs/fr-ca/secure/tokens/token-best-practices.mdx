...
Par défaut, nous utilisons `scope=openid` dans l’authentification par jeton pour éviter d’avoir un jeton trop volumineux. Vous pouvez contrôler toutes les demandes <Tooltip href="/docs/docs/fr-ca/glossary?term=openid" tip="OpenID
...
Les jetons d’actualisation s’accumulent en raison des tests automatisés et sont généralement utilisés pendant toute la durée du test. Pour éviter une accumulation de jetons soumise à des limites de jetons d’actualisation, utilisez l’Auth0 <Tooltip href="/docs/docs/fr-ca/glossary?term=management-api" tip="Management API
