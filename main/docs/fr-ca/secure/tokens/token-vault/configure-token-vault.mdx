...
Once a user authenticates with a [supported external provider](https://auth0.com/ai/docs/integrations/overview) and authorizes the connection, your application can <Tooltip href="/docs/docs/fr-ca/glossary?term=access-token" tip="Jeton d’accès
...
Use the <Tooltip href="/docs/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
Once you enable Token Vault for your connection, access and <Tooltip href="/docs/docs/fr-ca/glossary?term=refresh-token" tip="Jeton d’actualisation
Jeton utilisé pour obtenir un jeton d’accès renouvelé sans obliger les utilisateurs à se connecter à nouveau." cta="Voir le glossaire">refresh tokens</Tooltip> will no longer be stored in the user’s `identities` array. Instead, they will be stored in a secure <Tooltip href="/docs/docs/fr-ca/glossary?term=tokenset" tip="Tokenset
