...
2. Select the application you want to use with mTLS or [create a new application](/get-started/auth0-overview/create-applications).
3. Select the **Credentials** tab.
4. Choose the required **Authentication Method**, which can be either:

   * mTLS with a [self-signed certificate](/customize/custom-domains/self-managed-certificates)
...
The method of generating a CA-signed client certificate is highly dependent on the Public Key Infrastructure and outside of the scope of this document. We recommend generating at least a [2048-bit RSA key pair](/secure/application-credentials/generate-rsa-key-pair).
...
Once you have [generated a key pair](/secure/application-credentials/generate-rsa-key-pair) exclusively for mTLS, create the credential resource. Make the following POST request to the `/clients` endpoint:
