...
For additional information on this topic refer to [Select from Multiple Connection Options](/libraries/lock/selecting-from-multiple-connection-options).
...
**See the implementation in** [**ASP.NET Core**](/get-started/architecture-scenarios/sso-for-regular-web-apps/implementation-aspnetcore#configure-the-cookie-and-oidc-middleware).
...
To redirect a user after logout, add a `returnTo` query string parameter with the target URL as the value: `https://{yourDomain}/v2/logout?returnTo=http://www.example.com`. Note, that you will need to add the `returnTo` URL as an **Allowed Logout URLs**. For more information on how to implement this refer to: [Logout](/authenticate/login/logout).
...
* Add metadata to the user's profile by making use of [rules](/customize/rules).
...
For more information refer to the [Authorization Extension documentation](/customize/extensions/authorization-extension).
...
See the implementation in [ASP.NET Core](/get-started/architecture-scenarios/sso-for-regular-web-apps/implementation-aspnetcore#implement-admin-permissions).
