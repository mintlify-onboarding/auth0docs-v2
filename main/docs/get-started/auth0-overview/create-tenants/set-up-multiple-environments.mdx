...
Production [rate limits](/troubleshoot/customer-support/operational-policies/rate-limit-policy) only apply to tenants tagged as `Production`. Ensure your tenant's environment tag is set to `Production` before going live, and note the differences between rate limits for different tenants, environments, and fee structures.

You can name your multiple environments any way you prefer. For production environments, we strongly recommend using [custom domains](/customize/custom-domains).

If you have an Enterprise subscription plan, you can create child tenants by [linking multiple tenants to a single subscription](/get-started/auth0-overview/create-tenants/child-tenants); child tenants each gain access to the paid or upgraded features associated with the subscription. You can ensure your development, staging, or testing environments have access to the same features as your production environment by linking them all to the same Enterprise subscription.
...
Through the [Management API v2](https://auth0.com/docs/api/management/v2), you can automate the migration of assets ([rules](/customize/rules), database [connections](/authenticate/identity-providers), and so forth) between tenants.
