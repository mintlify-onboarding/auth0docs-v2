...
* [Mobile push notifications](/docs/get-started/applications/configure-client-initiated-backchannel-authentication/mobile-push-notifications-with-ciba) using the Auth0 Guardian app and a custom app integrated with the Auth0 Guardian SDK.
* [Email notifications](/docs/get-started/applications/configure-client-initiated-backchannel-authentication/email-notifications-with-ciba), which require setting a customized consent prompt.
...
* [Step 3: Client application polls for a response](#step-3%3A<<<
-client-application-polls-for-a-response)
...
* If you're using email notifications with CIBA with RAR, set a customized consent prompt.
...
<td>Tenant name that is passed within the <code>login_hint</code> structure. It can also be a custom domain. <br/><br/> **Example**: `login_hint={"format": "iss_sub", "iss": "https://$tenant/", "sub":"$user_id"}`</td>
</tr>
<tr>
...
<td>User ID for the authorizing user that is passed within the <code>login_hint</code> structure. If <code>iss_sub</code> format is used, then the user ID is passed within the <code>sub</code> claim. <br/><br/> **Example**: `login_hint={"format": "iss_sub", "iss": "https://$tenant/", "sub":"$user_id"}` <br/><br/>The user ID for a federated connection may have a different format.<br/></td>
</tr>
<tr>
...
Custom app renders the authorization_details in a custom consent screen using a push notification via the Auth0 Guardian SDK. To learn customize the consent screen, read Set customized consent prompt. 
* [Email notification](/docs/get-started/authentication-and-authorization-flow/client-initiated-backchannel-authentication-flow/email-notifications-with-ciba#step-6%3A-browser-sends-the-user-response-back-to-auth0): The browser renders the `authorization_details` in a custom consent screen. To learn how to customize the consent screen, read Set customized consent prompt.
