...
This tutorial will help you call your own API using the Hybrid Flow. If you want to learn how the flow works and why you should use it, see [Hybrid Flow](/get-started/authentication-and-authorization-flow/hybrid-flow).
...
* [Register your Application with Auth0](/get-started/auth0-overview/create-applications/regular-web-apps).

  + Select the appropriate **Application Type**.
  + Add an **Allowed Callback URL** of `{https://yourApp/callback}`.
  + Make sure your Application's **Grant Types** include **Implicit** and **Authorization Code**. To learn how, read [Update Grant Types](/get-started/applications/update-grant-types).
...
* [Register your API with Auth0](/get-started/auth0-overview/set-up-apis)
...
<td>A cryptographically random string that your app adds to the initial request and Auth0 includes inside the ID Token, <a href="/get-started/authentication-and-authorization-flow/implicit-flow-with-form-post/mitigate-replay-attacks-when-using-the-implicit-flow">used to prevent token replay attacks</a>.</td>
</tr>
<tr>
...
When you decode and parse your <Tooltip tip="ID Token: Credential meant for the client itself, rather than for accessing a resource." cta="View Glossary" href="/glossary?term=ID+token">ID token</Tooltip>, you will notice an additional claim, `c_hash`, which contains a hash of the `code`. This claim is mandatory when an ID token is issued at the same time as a `code`, and you should validate it:
...
<td>Your application's Client Secret. You can find this value in your <a href="https://manage.auth0.com/#/Applications/{yourClientId}/settings">Application Settings</a>.  To learn more about available application authentication methods, read <a href="/secure/application-credentials">Application Credentials</a>.</td>
</tr>
<tr>
<td><code>redirect_uri</code></td>
...
You can use rules to change the returned scopes of Access Tokens and/or add claims to Access and ID Tokens. (To learn more about rules, read [Auth0 Rules](/customize/rules).) To do so, add the following rule, which will run after the user authenticates:
