...
These links populate using your active tenant to provide you with accurate information. You must be logged in to [auth0.com/docs](/articles) with your tenant credentials to access these links.
...
By default, SAML assertions for IdP connections are signed, which we recommend. To get public keys you can use to configure the IdP, see [SAML Identity Provider Configuration: Signed Assertions](/authenticate/protocols/saml/saml-identity-provider-configuration-settings).
...
Rotating your signing key will be subject to a smaller rate limit than other API endpoints. To learn more, read [Management API Rate Limits](/troubleshoot/customer-support/operational-policies/rate-limit-policy/management-api-endpoint-rate-limits).
