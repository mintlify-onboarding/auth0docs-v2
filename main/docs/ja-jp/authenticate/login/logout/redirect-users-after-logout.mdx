...
2. `*`を[サブドメインのワイルドカードとして使用する](/ja-jp/get-started/applications/wildcards-for-subdomains)（例：`http://*.example.com`）
...
Auth0の`oidc/logout`エンドポイントを使ってリダイレクトする方法については、「[OIDCのエンドポイントを使ってユーザーをAuth0からログアウトさせる](/ja-jp/authenticate/login/logout/log-users-out-of-auth0)」をお読みください。
...
すべての<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/ja-jp/glossary?term=idp" tip="IDプロバイダー（IdP）: デジタルIDを保存および管理するサービス。" cta="用語集の表示">IdPs</Tooltip>が`returnTo`をサポートしているとは限りません。
