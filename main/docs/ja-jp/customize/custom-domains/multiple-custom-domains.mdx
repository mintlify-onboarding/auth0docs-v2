...
3. In the configuration form, provide the following information:
   ...
   * **Certificate type:** Choose [** Auth0-managed certificates** ](/ja-jp/customize/custom-domains/auth0-managed-certificates) or [**Self-managed certificates** ](/ja-jp/customize/custom-domains/self-managed-certificates).
...
To create a custom domain with the Management API, you need a machine-to-machine (M2M) application that is  authorized to use the  Management API. For more information, review. For more information, review [Management API Access Tokens](/ja-jp/secure/tokens/access-tokens/management-api-access-tokens).
...
7. Review [Get Management API Access Tokens](/ja-jp/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) to retrieve and store your access token.
...
After your custom domain has been verified by Auth0, you can use it immediately to configure Auth0 features for your users. For more information, read [Configure Features to Use Custom Domains](/ja-jp/customize/custom-domains/configure-features-to-use-custom-domains).
...
If you configured Multiple Custom Domains and enabled [Use Custom Domains in Emails](/ja-jp/customize/custom-domains/configure-features-to-use-custom-domains#use-custom-domains-in-emails), the `auth0-custom-domain` HTTP header is available when using the Auth0 Management API. The header is passed as the value for the `domain object` in email templates.
...
Auth0 [Actions](/ja-jp/customize/actions) allows you to create custom logic handling of your different transactions based on the custom domain.

For example, you could create an Action that directs a user to an associated [Organization](/ja-jp/manage-users/organizations), or enforce a specific [access control policy](/ja-jp/customize/actions/use-cases#api-authorization).
