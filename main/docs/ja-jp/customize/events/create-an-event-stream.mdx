...
6. Review [Get Management API Access Tokens](/ja-jp/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) to retrieve and store your access token.
...
Before setting up an event stream, you need to identify the [event types](/ja-jp/customize/events/event-types) you want to monitor. Then, you will use your AWS account ID and region to set up your event stream, as demonstrated below.
...
Before setting up an event stream, you need to identify the [event types](/ja-jp/customize/events/event-types) you want to monitor. You will then use your webhook handler to create an event stream, as demonstrated below.
...
After the stream is active, you can test the event stream. For more information, review [**Event Testing, Observability, and Failure Recovery** ](/ja-jp/customize/events/event-testing-observability-and-failure-recovery).
