...
インタースティシャルCAPTCHA画面クラスは[Identifier First認証](/ja-jp/authenticate/login/auth0-universal-login/identifier-first)フローの一部で、テナント設定を基に表示されます。詳細については、「[ボット検知](/ja-jp/secure/attack-protection/bot-detection)」を参照してください。
InterstitialCaptcha
