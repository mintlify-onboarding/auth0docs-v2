...
クライアントシークレットをパブリッククライアントアプリケーションに保管してはいけません。詳細については、「[機密アプリケーションとパブリックアプリケーション](/ja-jp/get-started/applications/confidential-and-public-applications)
...
1. Management API[クライアントシークレットのローテーション](https://auth0.com/docs/api/management/v2#!/Clients/post_rotate_secret)エンドポイントを呼び出します。`YOUR_CLIENT_ID`と`MGMT_API_ACCESS_TOKEN`のプレースホルダーの値を、それぞれクライアントIDとManagement APIのアクセストークンに置き換えてください。
   ...
   <td>スコープ`update:client_keys`を持つ<a href="/ja-jp/api/management/v2/tokens">Management APIのアクセストークン</a>。</td>
   </tr>
   </tbody>
   </table>
