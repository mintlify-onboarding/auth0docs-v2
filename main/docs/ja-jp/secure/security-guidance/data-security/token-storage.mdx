...
* [通常のWebアプリのクイックスタート](/ja-jp/quickstart/webapp)
...
* [Androidのトークンの保存と更新](/ja-jp/auth0-android-save-and-renew-tokens)
* [Swiftのトークンの保存と更新](/ja-jp/libraries/auth0-swift/auth0-swift-save-and-renew-tokens)
* [ネイティブ/モバイルアプリのQuickstarts](/ja-jp/quickstart/native)

</Tab><Tab title="シングルページアプリ">

トークン保存やセッション管理、その他の詳細に対処するには、[Auth0 SPA SDK](/ja-jp/libraries/auth0-single-page-app-sdk)の使用をお勧めします。
...
OAuth2の仕様に準拠して、ブラウザーが/tokenエンドポイントからリフレッシュトークンを要求したとき、Auth0は、そのクライアントに対して[リフレッシュトークンのローテーション](/ja-jp/secure/tokens/refresh-tokens/refresh-token-rotation)が有効になっている場合のみリフレッシュトークンを返します。
