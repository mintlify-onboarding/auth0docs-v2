...
2017年6月8日以降、新規テナントでは、ネイティブアプリケーションから行うパスワードレス認証がデフォルトで無効になります。代わりに、ユニバーサルログインを使用してWeb認証を行うことが推奨されています。引き続き利用したい場合は、まずダッシュボードで［MFA Grant Type（MFA付与タイプ）］を有効にする必要があります。詳細については、「[アプリケーションの付与タイプ](/docs/jp/get-started/applications/application-grant-types)」を参照してください。
...
最初に[Android SDKをセットアップ](/docs/jp/libraries/auth0-android)して、以下のパスワードレスメソッドを使えるようにします。
