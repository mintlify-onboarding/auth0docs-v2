...
<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip>を使用して認証方法にパスキーを有効化し、パスキーポリシーを構成することができます。
...
[［New Universal Login Experience（新しいユニバーサルログインエクスペリエンス）］](/docs/jp/authenticate/login/auth0-universal-login/universal-login-vs-classic-login/universal-experience)を有効にして構成します。

#### カスタムログインページ設定を無効にする

**［Customize Login Page（ログインページのカスタマイズ）］** 設定では、[カスタムドメイン](/docs/jp/customize/custom-domains)の使用に合わせてログインページをカスタマイズできます。
...
[［Identifier First authentication（Identifier First認証）］](/docs/jp/authenticate/login/auth0-universal-login/identifier-first)を有効にして構成します。
...
**［Use my own database（独自のデータベースを使用する）］** 設定では、外部データベースを使用して、[カスタムデータベース](/docs/jp/authenticate/database-connections/custom-db/create-db-connection)接続のユーザーを保管できます。

**［Import Users to Auth0（Auth0にユーザーをインポートする）］** 設定では、外部データベースからAuth0にユーザーを自動的にインポートできます。詳細については、「[ユーザーのインポートとエクスポート](/docs/jp/manage-users/user-migration/user-migration-scenarios)」をお読みください。
