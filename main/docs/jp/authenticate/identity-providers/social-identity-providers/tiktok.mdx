...
TikTokは`client_id`ではなく、`client_key`パラメーターを使用するため、認証時には<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>を使って`client_key`パラメーターを渡す必要があります。
