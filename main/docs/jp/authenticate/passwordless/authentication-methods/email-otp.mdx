...
詳細については、「[パスワードレス認証にメールまたはSMSを構成する](/docs/jp/authenticate/login/auth0-universal-login/passwordless-login/email-or-sms)」をお読みください。
...
4. **［Disable Sign Ups（サインアップを無効にする）］** かどうかを決定します。この設定を有効にすると、既存のユーザーに対してのみパスワードレスのアクセスを許可できますが、アプリケーションがユーザー列挙攻撃の脅威にさらされる可能性があります。詳しくは、[パスワードレス接続のベストプラクティス](/docs/jp/authenticate/passwordless/best-practices)をお読みください。
5. **［Save（保存）］** を選択します。
...
ユーザーが[組織](/docs/jp/manage-users/organizations/organizations-overview)を通じてログインしている場合は、次のパラメータも利用できます：
...
[独自のSMTPメールプロバイダーを構成](/docs/jp/customize/email/smtp-email-providers)して、メール通信を管理、監視、トラブルシューティングし、メールテンプレートをカスタマイズします。
...
* [カスタマイズされたSMTP外部メールプロバイダー](/docs/jp/customize/email/smtp-email-providers/configure-custom-external-smtp-email-provider)