...
[ルール](/docs/jp/customize/rules)を使用すると、[パスワードレス接続](/docs/jp/authenticate/passwordless)だけを使うよりも複雑な状況が扱えるようになります。たとえば、メールアドレスやデバイスの所有を確実にするために、さらに対策を追加することができます。

## 企業ネットワーク外部のユーザーに多要素認証を要求する

企業ネットワークの外部からパスワードレス接続を使ってアプリケーションにアクセスしているユーザーに、[多要素認証（MFA）](/docs/jp/secure/multi-factor-authentication)を必須にしたいとします。
...
これを行うには、[以下のルールを作成](/docs/jp/customize/rules/create-rules)します。
