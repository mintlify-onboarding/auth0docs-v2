...
MicrosoftのActive Directory Federation Services（ADFS）へのカスタム<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" href="/docs/jp/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language（SAML）: パスワードなしに二者間で認証情報を交換できる標準化プロトコル。" cta="用語集の表示">SAML</Tooltip>接続を作成して、マッピングを構成する際の柔軟性を高めます。カスタム接続を作成するには、次の操作を行う必要があります。
...
**［Add <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=relying-party" tip="証明書利用者: ユーザーを認証するためにサードパーティーのIDプロバイダーに依存するエンティティ（サービスやアプリケーションなど）。" cta="用語集の表示">Relying Party</Tooltip> Trust（証明者利用者信頼の追加）］** ウィザードを閉じると、 **［Edit Claim Issuance Policy（要求発行ポリシーの編集）］** ウィンドウが表示されます。
...
1. [Auth0がサービスプロバイダーとして機能する](/docs/jp/authenticate/protocols/saml/saml-sso-integrations/configure-auth0-saml-service-provider)SAML接続を作成するチュートリアルに従います。プロンプトが表示されたら、ADFSからエクスポートした署名証明書をアップロードします。
