...
トランザクションのセキュリティを強化するために、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" href="/docs/jp/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language（SAML）: パスワードなしに二者間で認証情報を交換できる標準化プロトコル。" cta="用語集の表示">SAML</Tooltip>プロトコルで要求と応答の両方に署名または暗号化できます。この記事では、特定のシナリオの構成について、2つのユースケースに分けて説明します。
...
Auth0がSAML **サービスプロバイダー** の場合、Auth0が<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" href="/docs/jp/glossary?term=idp" tip="IDプロバイダー（IdP）: デジタルIDを保存および管理するサービス。" cta="用語集の表示">IdP</Tooltip>に送信する認証要求に署名することができます。
...
ペイロードで使用するためにJSON文字列としてフォーマットされた秘密鍵と証明書を取得する方法については、「[証明書とキーと文字列の操作](/docs/jp/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings)」を参照してください。
...
秘密鍵と証明書ファイルをルールで使用できる文字列に変換する方法については、「[証明書とキーと文字列の操作](/docs/jp/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings)」を参照してください。
...
Auth0がSAML IDプロバイダーである場合は、[Actions](/docs/jp/customize/actions)を使用して送信するSAMLアサーションを暗号化できます。
...
証明書と公開鍵ファイルを取得したら、[それらを文字列に変換](/docs/jp/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings)してActionで使用する必要があります。Actionは次のようになります。
...
* [証明書やキーの文字列表現で作業する](/docs/jp/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings)