...
[IDプロバイダーのヘルプページ](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/okta)に記載の指示に従って、Okta Workforce Identity CloudをIDプロバイダーとして有効にします。
...
Okta Workforce Identity Cloudでの<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" href="/docs/jp/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language（SAML）: パスワードなしに二者間で認証情報を交換できる標準化プロトコル。" cta="用語集の表示">SAML</Tooltip>とSCIMの統合の構成については、「[Okta Workforce SAML接続のインバウンドSCIM](/docs/jp/authenticate/protocols/scim/inbound-scim-for-okta-workforce-saml-connections)」をお読みください。
...
2. [ログイン時に追加の属性を同期する](/docs/jp/authenticate/protocols/scim/configure-inbound-scim)場合を除き、 **［Sync user profile attributes at each login（ログインの度にユーザープロファイル属性を同期する）］** を無効にします。
