...
移行を開始する前に、「[RulesからActionsに移行する](/docs/jp/customize/actions/migrate/migrate-from-rules-to-actions)」をご覧ください。.この移行ガイドは、ルールとアクションの違いが説明されている優れたリソースであり、移行過程においても役立つツールです。
...
`api.rules.wasExecuted()` 関数の詳細については、「[アクショントリガー: post-login - APIオブジェクト](/docs/jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object)」を参照してください。
...
アクションは移行過程の重要な一部であるため、エディターで直接テストすることができます。詳細については、「[アクションをテストする](/docs/jp/customize/actions/test-actions)」を参照してください。
...
フローにアクションを追加する方法については、「[初めてアクションを作成する](/docs/jp/customize/actions/write-your-first-action)」を参照してください。
