...
拡張機能で認可ロジックを適用する前に、ログイントランザクション中の動作を構成する必要があります。構成設定は、実行時に実行される[ルール](/docs/jp/customize/rules)にキャプチャされます。

## 前提条件

[認可拡張機能のインストール](/docs/jp/customize/extensions/authorization-extension/migrate-to-authorization-extension-v2)
...
データはユーザーの`app_metadata`フィールドに保存され、ユーザーがログインした後、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>または[Dashboard](https://manage.auth0.com/#/users)を使用してこの情報を取得できます。
...
* [認可拡張機能v2に移行する](/docs/jp/customize/extensions/authorization-extension/migrate-to-authorization-extension-v2)