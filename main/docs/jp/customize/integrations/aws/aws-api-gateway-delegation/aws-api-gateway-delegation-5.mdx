...
* <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=openid" tip="OpenID: アプリケーションがログイン情報を収集および保存することなくにユーザーのIDを検証できるようにする認証用のオープン標準。" cta="用語集の表示">OpenID</Tooltip>の<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" href="/docs/jp/glossary?term=json-web-token" tip="JSON Web Token（JWT）: 二者間のクレームを安全に表現するために使用される標準IDトークン形式（および多くの場合、アクセストークン形式）。" cta="用語集の表示">JSON Web Token</Tooltip>（JWT）を渡して、サービスへIDを送る
* トークンを検証する
...
ユーザーのメールアドレスをJWTに追加する方法の1つが、[ルール](/docs/jp/rules)です。この方法は、値がユーザー認証用のJWTで常に利用可能なことを確実にしたい場合には適切です。
...
トークン検証の詳細については、「[Auth0が対応しているIDプロトコル](/docs/jp/authenticate/protocols)」をご覧ください。

秘密鍵で`auth0-variables.js`を更新します。秘密鍵は、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip>にある［Application（アプリケーション）］の［Settings（設定）］タブに表示されます。
...
* APIへのアクセスをユーザーベースと結び付けて、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" href="/docs/jp/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language（SAML）: パスワードなしに二者間で認証情報を交換できる標準化プロトコル。" cta="用語集の表示">SAML</Tooltip> IDプロバイダーとIAMを統合する
