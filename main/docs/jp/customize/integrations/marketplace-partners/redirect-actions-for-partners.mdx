...
2. ログインが成功すると、[Post Loginトリガー](/docs/jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger)内のすべてのアクションが実行されます（MFAが有効な場合は、MFAの前に実行されます）。
...
* Auth0からリダイレクトするタイミングについて、どのように判断するのか?
  ...
  + [ユーザーのapp_metadata](/docs/jp/manage-users/user-accounts/metadata/manage-metadata-rules)でフラグを立てるのか?
...
* [ユーザーのapp_metadata](/docs/jp/manage-users/user-accounts/metadata/manage-metadata-rules)または[正規化ユーザープロファイル](/docs/jp/manage-users/user-accounts/user-profiles/user-profile-structure)
* [レート制限](/docs/jp/troubleshoot/customer-support/operational-policies/rate-limit-policy/management-api-endpoint-rate-limits)に注意し、必要なときだけ更新する
* [カスタムトークンクレーム](/docs/jp/get-started/apis/scopes/sample-use-cases-scopes-and-claims)を使って、要求側アプリケーションに情報をどのように戻すのか?

これらすべての質問等に対する回答については、「[アクションを使ってリダイレクトする](/docs/jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions)」をお読みください。アクション統合を送信する準備ができたら、「[パートナー向けアクション統合](/docs/jp/customize/integrations/marketplace-partners/actions-integrations-for-partners)