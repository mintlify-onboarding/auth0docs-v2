...
高度なレンダリングモードは[Management API](/docs/jp/api/management/v2)のコールを通じて画面ごとに構成されるもので、[Deploy CLI](/docs/jp/deploy-monitor/deploy-cli-tool)と[Auth0 Terraform Provider](/docs/jp/deploy-monitor/auth0-terraform-provider)でもサポートされています。
...
以下は、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>を直接呼び出してログインID画面を設定する例です。
