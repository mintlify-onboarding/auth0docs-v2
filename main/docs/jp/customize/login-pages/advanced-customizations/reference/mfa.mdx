...
<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="多要素認証 (MFA): ユーザー名とパスワードに加えて、SMS 経由のコードなどの要素を使用するユーザー認証プロセス。" cta="用語集の表示" href="/docs/jp/glossary?term=multifactor-authentication">MFA</Tooltip>登録開始オプション画面クラスはMFA要素の選択と登録を処理します。
