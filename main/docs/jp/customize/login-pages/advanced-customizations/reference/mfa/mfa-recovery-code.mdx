...
The <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="多要素認証 (MFA): ユーザー名とパスワードに加えて、SMS 経由のコードなどの要素を使用するユーザー認証プロセス。" cta="用語集の表示" href="/docs/jp/glossary?term=multifactor-authentication">MFA</Tooltip> Recovery Code Challenge screen class provides methods associated with the mfa-recovery-code-challenge screen. This screen is displayed when the user needs to enter a recovery code to log in.
