...
このように保管された値には、指定された`ttl`または`expires_at`値までのライフタイムがあります。ライフタイムが指定されない場合には、デフォルトのライフタイムである15分が使用されます。ライフタイムは[アクションのキャッシュ制限](https://auth0.com/docs/jp/customize/actions/limitations)が定める最大値を超過してはいけません。
...
このイベント[`(fn)`](https://auth0.com/docs/jp/deploy-monitor/logs/log-event-type-codes)はエラーとしてログに記録されますが、その後、アクションには送信されません。
...
このイベント[`(fn)`](https://auth0.com/docs/jp/deploy-monitor/logs/log-event-type-codes)はエラーとしてログに記録され、その後、数分にかけて5回まで再試行されます。
