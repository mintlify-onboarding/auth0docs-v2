...
The Unified Phone Experience does not use the [custom SMS gateway](/docs/jp/authenticate/passwordless/authentication-methods/use-sms-gateway-passwordless) you configured for Passwordless SMS. You can use the <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip> or <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip> to configure a custom phone provider with Auth0 Actions. To learn more, read [Configure a Custom Phone Provider](/docs/jp/customize/phone-messages/configure-phone-messaging-providers/configure-a-custom-phone-provider).
