...
アプリケーションに対してクライアントが開始するバックチャネル認証を構成する方法について説明します。詳細については、「[クライアントが開始するバックチャネル認証フロー](/docs/jp/get-started/authentication-and-authorization-flow/client-initiated-backchannel-authentication-flow)」をお読みください。
...
CIBAのプッシュ通知を送信するには、テナントに対して[Auth0 Guardianのプッシュ通知](/docs/jp/secure/multi-factor-authentication/auth0-guardian#enroll-in-push-notifications)を有効にする必要があります。CIBAのプッシュ通知チャレンジを承認するには、認証を受けるユーザーもAuth0 Guardianのプッシュ通知要素に登録されている必要があります。詳細については、「[CIBAを使ったユーザー認証](/docs/jp/get-started/authentication-and-authorization-flow/client-initiated-backchannel-authentication-flow/user-authentication-with-ciba)」をお読みください。

<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip>を使用して、テナントに対してAuth0 Guardianのプッシュ通知を有効にします。

Auth0 Dashboardで以下を行います。

1. ［**Security（セキュリティ）>Multi-factor Auth（多要素認証）** ］を選択します。

2.［**Push Notification using Auth0 Guardian（Auth0 Guardianを使ったプッシュ通知）** ］を有効にします。このために<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=multifactor-authentication" tip="多要素認証（MFA）: ユーザー名とパスワードに加えて、SMS経由のコードなどの要素を使用するユーザー認証プロセス。" cta="用語集の表示">MFA</Tooltip>の構成設定が必要になる場合があります。詳細については、「[MFAにプッシュ通知を構成する](/docs/jp/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-push-notifications-for-mfa)」を参照してください。
...
Auth0のGo Management API SDKライブラリーを使用することもできます。詳細については、「[SDK](/docs/jp/libraries)」をお読みください:
