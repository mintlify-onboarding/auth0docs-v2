...
[Android App Links](https://developer.android.com/training/app-links)を使用すると、アプリケーションは自身を特定の種類のリンクのデフォルトハンドラーとして指定できます。たとえば、メールにあるURLをクリックすると、指定されたアプリケーションでリンクが開きます。このガイドでは、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip>を使用して、Auth0登録済みアプリケーションのAndroidアプリリンクサポートを有効にする方法を説明します。
