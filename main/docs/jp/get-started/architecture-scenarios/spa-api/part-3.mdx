...
安全のため、ユーザーアクセストークンのライフタイムは短くすることが推奨されます。<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip>でAPIを作成する場合、デフォルトのライフタイムは`7200`秒（2時間）ですが、APIごとに制御できます。
