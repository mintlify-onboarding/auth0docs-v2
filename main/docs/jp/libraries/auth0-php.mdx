...
Auth0認証と<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>を使用するには、無料のAuth0アカウントとアプリケーションが必要です：
...
4. 必要な資格情報に使用する **［Settings（設定）］** タブをクリックします。詳細については、[「アプリケーションの設定」](/docs/jp/get-started/applications/application-settings)をお読みください。
