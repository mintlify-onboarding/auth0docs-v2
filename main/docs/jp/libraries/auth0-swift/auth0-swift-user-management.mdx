...
<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>は[ユーザーアカウントのリンク](/docs/jp/manage-users/user-accounts/user-account-linking)を提供して、ユーザーがさまざまなプロバイダーから取得した個別のユーザーアカウントを1つのプロファイルにリンクまたはリンク解除できるようにます。また、ユーザーメタデータや他のプロファイル情報を更新することもできます。
