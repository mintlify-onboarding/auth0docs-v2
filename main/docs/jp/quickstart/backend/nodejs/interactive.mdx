...
<Recipe>
  <Content>
    ...
    Auth0 Dashboardを使って初めてAPIをセットアップする場合には、[使用の開始ガイド](/docs/jp/get-started/auth0-overview/set-up-apis)を確認してください。それぞれのAuth0 APIにはAPI識別子があり、アプリケーションにアクセストークンの検証で使用されます。

    <Info>
    **Auth0を初めてご利用ですか？** [Auth0の仕組み](/docs/jp/overview)と、OAuth 2.0フレームワークを用いた[API認証と認可の実装](/docs/jp/api-auth)について説明します。
    </Info>
