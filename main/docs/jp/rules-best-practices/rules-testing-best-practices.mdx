...
<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip>には、ルールをテストしてデバッグするために、ルールを`TRY`（試す）機能があります。この機能では、モックの`user`と`context`オブジェクトを定義し、ルールに渡して実行することができます。ルールの実行が完了すると、結果の出力（コンソールのログを含む）が表示されます。これはルールの単体テストを一目で分かりやすいものにする反面、手動での作業がメインとなるため、[Mocha](https://mochajs.org/)や[rewire](https://www.npmjs.com/package/rewire)などのテスト自動化ツールは活用できません。
