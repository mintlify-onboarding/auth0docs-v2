...
Auth0のキーの詳細については、「[Auth0のキー階層](/docs/jp/secure/highly-regulated-identity/customer-managed-keys#auth0-key-hierarchy)」をお読みください。
...
[Dashboard](/docs/jp/secure/highly-regulated-identity/customer-managed-keys/customer-managed-keys-dashboard)または[Management API](/docs/jp/secure/highly-regulated-identity/customer-managed-keys/customer-managed-keys-management-api)を使用して、BYOKを有効にすることができます。

* デフォルトのAuth0が生成した環境ルートキーを新しい顧客提供ルートキーに置き換える。
* キー階層を顧客提供ルートキーでローテーションし、再暗号化する。たとえば、新しいテナントマスターキーと新しい名前空間キーを作成して再暗号化する。

カスタマーマネージドキー操作を行うと、[テナントログ](/docs/jp/deploy-monitor/logs)に以下のログイベントが追加されます。
