...
プライバシー上の理由により Auth0 データベースに保存すべきでないユーザーフィールドがある場合は、それらを拒否リストに追加できます。拒否リストに属性を追加するには、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>の更新接続エンドポイントに対して`PATCH`呼び出しを行います。

1. `/patch_connections_by_id`[エンドポイント](/docs/jp/api/management/v2#!/Connections/patch_connections_by_id)にアクセスするための有効なアクセストークンを取得します。トークンには、`update:connections`スコープが含まれている必要があります。詳細については、[［Management API Access Tokens（Management APIのアクセストークン）］](/docs/jp/secure/tokens/access-tokens/management-api-access-tokens)を参照してください。
...
```
   ...
   ここでは、
   1. `{yourConnectionId}` は、これらの属性が拒否される[接続 ID](/docs/jp/authenticate/identity-providers/locate-the-connection-id)です。
...
* 拒否できるのは、[ルート フィールド](/docs/jp/manage-users/user-accounts/user-profiles/user-profile-structure)（`user.name`や`user.phone_number` など）のみです。
...
これらの制限のいずれかが受け入れられない場合は、データを暗号化する[ルール](/docs/jp/customize/rules)を記述し、データを`user.app_metadata`オブジェクトに保持することができます。

## もっと詳しく

* [許可リスト用のAuth0 IPアドレス](/docs/jp/secure/security-guidance/data-security/allowlist)