...
[AD/LDAPコネクタ](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector)で問題が発生している場合は、以下をお読みいただいて、一般的な問題のトラブルシューティング方法を確認してください。

## トラブルシューティングツールを実行

トラブルシューティングツールは、AD/LDAPコネクタ管理コンソールで実行することも、Admin Consoleの外部でプログラムを手動で実行することもできます。

証明書の問題を検出するには、[AD/LDAPコネクタ構成ファイル](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-config-file-schema)で`CONNECTIONS_API_V2_KEY`変数を設定します。
...
ユーザープロファイルにグループを返す必要がない場合、Auth0は、[AD/LDAP Connector構成ファイル](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-config-file-schema)で`GROUPS`変数を明示的に無効にすることを推奨します。
...
影響を軽減するには、AD/LDAPサーバーで再帰的または問題のあるグループ割り当てを修正するか、[AD/LDAPコネクタ構成ファイル](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-config-file-schema)の`GROUPS_CACHE_SECONDS`変数の値を増やします。

#### アクティブユーザーのボリュームを評価

最初にAD/LDAPコネクタを展開してから、アクティブユーザーの数が増加しているかどうかを確認します。これは、独自の[監視ソリューション](/docs/jp/customize/log-streams)を使用して評価することも、[Auth0アクティブユーザーレポート](/docs/jp/troubleshoot/customer-support/manage-subscriptions/monitor-subscription-usage)から推定値を取得することもできます。
...
アクティブユーザーの数が増加し、上記のすべての手順が既に実行されている場合は、ホストマシンのハードウェア仕様をアップグレードするか、[高可用性環境](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-high-availability)を構成する必要があります。
...
AD/LDAPコネクタは、ユーザーのグループメンバーシップをキャッシュします。このキャッシュの有効期限は[AD/LDAPコネクタ設定ファイル](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-config-file-schema)の`GROUPS_CACHE_SECONDS`変数によって決定され、デフォルト値は600（秒）です。
...
[カスタムドメイン](/docs/jp/customize/custom-domains)の追加の構成を完了していない場合、このエラーが発生することがあります。

KerberosをサポートするAD/LDAP接続を使用するには、カスタムドメインで動作するように **Ticket（チケット）］** エンドポイントを更新する必要があります。

1. [AD/LDAPコネクタ構成ファイル](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-config-file-schema)を開きます。
...
[高可用性環境](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-high-availability)を構成しており、追加マシンでこのエラーが発生する場合、追加マシンの信頼されたルート認証機関が初期マシンの信頼されたルート認証機関と一致していることを確認してください。

## Auth0サポートにお問い合わせ

これらの問題を自分で解決できない場合は、[Auth0サポート](/docs/jp/troubleshoot/customer-support/open-and-manage-support-tickets)にお問い合わせください。

サポートチームが問題をトラブルシューティングできるように、サポートチケットに次の項目を含めてください。

1. 課題の説明。
2. [AD/LDAP構成ファイルのエクスポート](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/import-export-ad-ldap-connector-configs)。
3. サービスログファイルのコピー:
...
* [AD/LDAPコネクターのシステム要件](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-requirements)
* [AD/LDAPコネクター構成のインポートとエクスポ―ト](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/import-export-ad-ldap-connector-configs)
* [AD/LDAPコネクターのテスト環境をセットアップする](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-test-environment)
* [AD/LDAPコネクターを更新する](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/update-ad-ldap-connectors)
* [AD/LDAPコネクターの構成ファイルスキーマ](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-config-file-schema)
* [System Center Operations Managerを使用してAD/LDAPコネクターを監視する](/docs/jp/authenticate/identity-providers/enterprise-identity-providers/active-directory-ldap/ad-ldap-connector/ad-ldap-connector-scom)