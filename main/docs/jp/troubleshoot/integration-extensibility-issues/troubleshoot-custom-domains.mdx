...
カスタムドメインを構成するときにこのエラーが表示された場合は、セットアップに応じて異なる追加の構成を実行する必要があります。詳しくは、[カスタムドメインを使用するための機能を構成する](/docs/jp/customize/custom-domains/configure-features-to-use-custom-domains)をお読みください。
...
同様に、[Auth0 Management API](/docs/jp/api/management/v2)の呼び出しでカスタムドメインを使用すると、同じ理由でエラーが発生します。
...
あるいは、[Auth0 ユニバーサルログイン](/docs/jp/authenticate/login/auth0-universal-login)を実装することで、クロスオリジン認証への依存を排除​​することもできます。
...
* [TLS（SSL）のバージョンと暗号](/docs/jp/customize/custom-domains/self-managed-certificates/tls-ssl)