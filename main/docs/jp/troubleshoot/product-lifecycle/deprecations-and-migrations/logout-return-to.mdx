...
* `https://{yourDomain}/v2/logout`：`federated`クエリパラメーターを使用して、[IDプロバイダーからユーザーをログアウト](/docs/jp/authenticate/login/logout/log-users-out-of-idps)
* `https://{yourDomain}/wsfed/{yourClientId}?wa=wsignout1.0`：`wreply`クエリを使用して、[WS-FedプロトコルでAuth0からログアウト](/docs/jp/authenticate/protocols/ws-fed-protocol)
...
2. 影響を受けるアプリケーションのそれぞれが以下であることを確実にします。
   ...
   1. アプリケーションまたは証明書利用者が、Auth0の公開APIの[Authentication API:Logout](/docs/jp/api/authentication#logout)または[Authentication API:WS-Federation](/docs/jp/api/authentication#ws-federation)を使用して、ログアウトを開始する。
