...
Instagramデータにアクセスする必要がある場合は、別の方法でユーザーを認証し（たとえば、Facebookログインや、ユーザー名とパスワードを使用）、Instagram <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/jp/glossary?term=oath2" tip="OAuth 2.0: 認可プロトコルとワークフローを定義する認可フレームワーク。" cta="用語集の表示">OAuth</Tooltip>フローをアプリケーションに実装します。