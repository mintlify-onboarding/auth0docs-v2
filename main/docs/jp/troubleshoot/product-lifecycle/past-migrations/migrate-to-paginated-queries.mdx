...
旧バージョンの[Auth0拡張機能](/docs/jp/customize/extensions)およびカスタムの拡張機能を、ページネーションクエリのみ実行するようにするために、最新バージョンに更新する必要があるかもしれません。
...
`auth0-deploy-cli`ツールを使用している場合は、バージョン**5.3.0以降** に更新する必要があります。詳細については、[Deploy CLIツール](/docs/jp/deploy-monitor/deploy-cli-tool)を参照してください。