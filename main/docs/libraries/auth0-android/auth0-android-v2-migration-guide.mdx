...
Learn more about the [OpenID Connect Protocol](/authenticate/protocols/openid-connect-protocol).

## Authorization Code with PKCE

v2 only supports the [Authorization Code with Proof Key for Code Exchange (PKCE)](/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce) flow. We removed the `withResponseType(@ResponseType int type)` method on `WebAuthProvider.Builder`.
