...
---
When deleting an [organization](/manage-users/organizations/organizations-overview), the association between all users’ membership and the deleted organization will be removed, though the users themselves will not be deleted from your tenant.
...
| `MGMT_API_ACCESS_TOKEN` | [Access Token for the Management API](/secure/tokens/access-tokens/management-api-access-tokens) with the scope `delete:organizations`. |
