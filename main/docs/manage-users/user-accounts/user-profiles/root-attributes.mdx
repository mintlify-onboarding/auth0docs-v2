...
---
Auth0's normalized user profile contains a number of root attributes (attributes stored at the first, or root, level of the object), some of which may be updated. To learn more about the user profile structure and its attributes, read [User Profile Structure.](/manage-users/user-accounts/user-profiles/user-profile-structure)

Methods for updating root attributes vary depending on connection type.

## Auth0 as the identity provider

When Auth0 is the <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/glossary?term=Identity+Provider">Identity Provider</Tooltip> (IdP), subscribers may:

* Set root attributes [on user sign-up](https://auth0.com/docs/api/management/guides/users/set-root-attributes-user-signup) (via the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip> or via public signup)
...
* [Regular database connections](/authenticate/database-connections)
* [Custom database connections](/authenticate/database-connections/custom-db) with import mode
* [Passwordless connections](/authenticate/passwordless)
...
* [Social connections](/authenticate/identity-providers#social)
* [Enterprise connections](/authenticate/identity-providers#enterprise)
* [Legal identity connections](/authenticate/identity-providers#legal-identities)
