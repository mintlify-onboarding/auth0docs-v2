...
* You need a token to make requests to the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip>. To learn more, read [Management API Access Tokens](/secure/tokens/access-tokens/management-api-access-tokens).
...
* To get the latest search results, use an **immediately consistent** endpoint during authentication processes, such as [Get Users by ID](/manage-users/user-search/retrieve-users-with-get-users-by-id-endpoint) and [Get Users by Email](/manage-users/user-search/retrieve-users-with-get-users-by-email-endpoint). Searches using these endpoints reflect the results of all successful write operations including those that occurred shortly prior to your request.
