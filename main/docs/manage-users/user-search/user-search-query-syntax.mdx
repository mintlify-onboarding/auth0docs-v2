...
* A term without a field name will not match text in the [user metadata](/manage-users/user-accounts/metadata) fields.
...
You can search for users using all the [normalized user profile fields](/manage-users/user-accounts/user-profiles/normalized-user-profile-schema) and the fields below:
...
<td>date time</td>
<td>The timestamp of when the user last logged in. In case this property executes from inside a <a href="/customize/rules">Rule</a> with the <code>user</code> object, the value will be associated with the login that triggered the rule (since rules execute after the actual login).</td>
</tr>
<tr>
...
When searching for users in the Auth0 <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip>, you can filter users by `user_metadata` or `app_metadata`. To do so, you can use Lucene Search Syntax with the `q` parameter.
...
* [Retrieve Users with the Get Users Endpoint](/manage-users/user-search/retrieve-users-with-get-users-endpoint)