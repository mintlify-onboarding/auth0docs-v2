...
To use one of the supported third-party CAPTCHA provider integrations, you need the provider’s configuration details. To learn more, read [Configure third-party CAPTCHA provider integrations](/secure/attack-protection/bot-detection/configure-captcha).
...
If you do not configure **Response** settings with Bot Detection enabled, Bot Detection operates in [Monitoring](/secure/attack-protection#monitoring) mode. Monitoring mode records related events (with risk assessment information) in your tenant log for you to review. To learn more, read [View Attack Protection Log Events](/secure/attack-protection/view-attack-protection-events).
...
5. If you choose **When Risky** or **Always**, the **CAPTCHA Providers** field will appear in the **Response** section. Select **Auth Challenge** (provided by Auth0), **Simple CAPTCHA** (provided by Auth0), or one of the supported third-party provider integrations (requires external setup and registration).
   ...
   * If you choose one of our third-party provider integrations, enter the provider’s configuration details. To learn more, read [Configure third-party CAPTCHA provider integrations](/secure/attack-protection/bot-detection/configure-captcha).
   * If you choose **Simple CAPTCHA,** the CAPTCHA image is not legible to screen readers.
6. If you choose **When Risky**, the **Bot Detection Level** field will appear in the **Response** section. Select the security level that best fits your use case. For more information, read [Configure Bot Detection Level](/secure/attack-protection/bot-detection#configure-bot-detection-level).
7. Select **Save**.
...
You can configure the detection model that Bot Detection uses for signup flows when using a [Custom Login Page](/customize/login-pages) or the [Classic Login Experience](/authenticate/login/auth0-universal-login/universal-login-vs-classic-login/classic-experience) in the <Tooltip tip="Auth0 Dashboard: Auth0's main product to configure your services." cta="View Glossary" href="/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>.
...
Bot Detection works for web and mobile applications that use [Auth0 Universal Login](/authenticate/login/auth0-universal-login). For applications that do not use <Tooltip tip="Universal Login: Your application redirects to Universal Login, hosted on Auth0's Authorization Server, to verify a user's identity." cta="View Glossary" href="/glossary?term=Universal+Login">Universal Login</Tooltip>, levels of support are limited, in particular for flows that cannot support a CAPTCHA or reCAPTCHA challenge.
...
Your custom login form code must handle scenarios where the user is asked to pass a CAPTCHA step. To learn more, read [Add Bot Detection to Custom Login Pages](/secure/attack-protection/bot-detection/bot-detection-custom-login-pages).
...
Your custom login form code must handle scenarios where the user is asked to pass a CAPTCHA step. To learn more, read [Add Bot Detection to Native Applications](/secure/attack-protection/bot-detection/bot-detection-native-apps).

## Learn more

* [Configure Third-Party CAPTCHA Provider Integrations](/secure/attack-protection/bot-detection/configure-captcha)
* [Add Bot Detection to Custom Login Pages](/secure/attack-protection/bot-detection/bot-detection-custom-login-pages)
* [Add Bot Detection to Native Applications](/secure/attack-protection/bot-detection/bot-detection-native-apps)
