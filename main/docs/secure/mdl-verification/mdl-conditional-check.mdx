...
You can store the response of a Verification Request, either with Auth0’s [metadata solution](/manage-users/user-accounts/metadata) or with your own storage solution. The storage of this data can be used to conditionally check if a verification request is needed or can be skipped. To learn more, read [Configure Mobile Driver’s License Verification Presentation Request](/secure/mdl-verification/configure-mdl-verification-request).
...
We recommend the use of [Actions](/manage-users/user-accounts/metadata/manage-user-metadata) as a means to check if a previous Verification Request was performed, is still valid, and/or the data obtained from a previous request is sufficient.