...
---
To get a <Tooltip tip="Refresh Token: Token used to obtain a renewed Access Token without forcing users to log in again." cta="View Glossary" href="/glossary?term=refresh+token">refresh token</Tooltip>, you must include the `offline_access` [scope](/get-started/apis/scopes) when you initiate an authentication request through the `/authorize` endpoint. Be sure to initiate Offline Access in your API. For more information, read [API Settings](/get-started/apis/api-settings).

For example, if you are using the [Authorization Code Flow](/get-started/authentication-and-authorization-flow/authorization-code-flow), the authentication request would look like the following:
...
For more information on how to implement this using the Authorization Code Flow, refer to our tutorial, [Call API Using the Authorization Code Flow](/get-started/authentication-and-authorization-flow/authorization-code-flow/call-your-api-using-the-authorization-code-flow). For other grants, see [Authentication and Authorization Flows](/get-started/authentication-and-authorization-flow).
