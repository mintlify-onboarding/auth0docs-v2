...
1. [Configure Connected Accounts for Token Vault](/docs/secure/tokens/token-vault/configure-token-vault#configure-connected-accounts) for a supported social or enterprise connection.
...
You can configure Connected Accounts for supported social and enterprise connections. To learn more, read [Configure Connected Accounts](/docs/secure/tokens/token-vault/configure-token-vault#configure-connected-accounts).
...
To configure your application for the refresh token exchange, make a `PATCH` call to the [Update a Client](/docs/api/management/v2/clients/patch-clients-by-id) endpoint to add the `refresh_token`, `authorization_code,` and `urn:auth0:params:oauth:grant-type:token-exchange:federated-connection-access-token` grant types to the client JSON object:
...
To configure your SPA, make a `PATCH` call to the [Update a Client](/docs/api/management/v2/clients/patch-clients-by-id) endpoint to add the `authorization_code` grant type to the client JSON object:
...
<td>The application type of your Custom API Client. To register the client as a resource server, set to resource_server.</td>
</tr>
<tr>
