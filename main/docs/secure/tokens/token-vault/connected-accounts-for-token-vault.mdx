...
3. In **Purpose**, toggle on **Use for Connected Accounts for Token Vault**. Depending on your **Purpose** setting, Auth0 may enable `offline_access` by default, allowing the client application to obtain an Auth0 refresh token. To learn more, read [User authentication vs Connected Accounts](#user-authentication-vs-connected-accounts).
5. Click **Save**.
