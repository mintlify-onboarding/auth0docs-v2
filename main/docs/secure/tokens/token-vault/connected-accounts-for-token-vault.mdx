...
<tr>
<td>User profile</td>
<td>Adds user account to `identities` array</td>
<td>Adds user account to `connected_accounts` array</td>
</tr>
<tr>
<td>Purpose</td>
...
//include screenshot when finalized
...
<td>Use the Auth0 Dashboard or Management API to select the desired scopes for your connection. At runtime, this list is automatically completed with any additional scopes included in the `connection_scope` parameter of the authorization request.<br/><br/>**Note:** By default, `offline_access` is not enabled for the connection.</td>
</tr>
<tr>
<td>Connected Accounts for Token Vault</td>
<td>The connection uses the Connected Accounts flow to retrieve and store the access tokens for the connection in the Token Vault. The connection does not use the `/authorize` login flow to authenticate users, and is excluded from the list of valid identity providers.</td>
<td>Use the Auth0 Dashboard or Management API to select the desired scopes for your connection. At runtime, any scopes included in the `scopes` parameter in the authorization request take precedence over the scopes selected in the Auth0 Dashboard.<br/><br/>**Note:** By default, `offline_access` is enabled for the connection.</td>
</tr>
<tr>
<td>Authentication and Connected Accounts for Token Vault</td>
...
<td>Use the Auth0 Dashboard and Management API to pass scopes to the connection using the `scopes` parameter in the authorization request. At runtime, any scopes included in the `scopes` parameter take precedence over the scopes selected in the Auth0 Dashboard.<br/><br/>**Note:** The user is prompted to consent to enabling offline_access. If the user accepts, `offline_access` is enabled for the connection.</td>
</tr>
</tbody>
</table>
...
1. The client application makes a `POST` request to the My Account API’s `/me/v1/connected-accounts` endpoint, passing scopes and other parameters in the scopes to send to the external provider. To learn more, read [Initiate Connected Accounts request](#initiate-connected-accounts-request).
...
Select Update. This creates a [client grant](/docs/get-started/applications/application-access-to-apis-client-grants) that allows your client application to access the My Account API with the Connected Accounts scopes on the user’s behalf. 
5. If you’re using [Multi-Resource Refresh Token](/docs/secure/tokens/refresh-tokens/multi-resource-refresh-token#multi-resource-refresh-token), navigate to the **Settings** tab. Under **Access Settings**, select **Allow Skipping User Consent**.
...
While you can use any [authentication and authorization flow](/docs/get-started/authentication-and-authorization-flow) that uses the `/authorize` endpoint to get an access token for the My Account API, Auth0 recommends using MRRT for native applications with the Authorization Code Flow with PKCE.
...
3. In **Purpose**, toggle on **Connected Accounts for Token Vault** or **Authentication and Connected Accounts for Token Vault**.
4. Depending on your **Purpose** setting, Auth0 may enable offline access by default so the client application can get an Auth0 refresh token. To learn more, read [User authentication vs Connected Accounts](#user-authentication-vs-connected-accounts).
5. Click **Save**.
...
```bash lines
curl -L -X PATCH "https://${account.namespace}/api/v2/connections/{connectionId}" \
-H 'Content-Type: application/json' \
...
While you can use any [authentication and authorization flow](/docs/get-started/authentication-and-authorization-flow) that uses the `/authorize` endpoint, Auth0 recommends using [Multi-Resource Refresh Token (MRRT)](/docs/secure/tokens/refresh-tokens/multi-resource-refresh-token) with the Authorization Code Flow with PKCE for public clients. By default, the [AI Framework SDKs](https://auth0.com/ai/docs/sdks/overview) use MRRT to get an access token for the My Account API.

The following sections explain how to use Multi-Resource Refresh Token (MRRT) to get an access token for the My Account API.
...
The following is an authorization code flow request for a confidential client that includes the `offline_scope` to return a refresh token and a single-use authorization code for the `https://my-example-api.com`: 

```bash lines
open "https://${account.namespace}/authorize?client_id=$<CLIENT_ID>&response_type=code&prompt=login&scope=openid%20profile%20offline_access&redirect_uri=<REDIRECT_URI>&state=<STATE>&audience=https://my-example-api.com"
```
  ...
  --data-binary @- <<EOF | jq -r '.refresh_token'
{
  ...
  "audience": "https://my-example-api.com",
...
Once you’ve obtained a refresh token, exchange it for an access token with a difference audience (the My Account API) and the Connected Accounts scopes using the refresh token grant type:

```bash lines
curl -s -X POST "https://${account.namespace}/oauth/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  --data-urlencode "grant_type=refresh_token" \
  --data-urlencode "client_id=<CLIENT_ID>" \
  --data-urlencode "client_secret=<CLIENT_SECRET>" \
  --data-urlencode "refresh_token=<REFRESH_TOKEN>" \
  --data-urlencode "audience=https://${account.namespace}/me/" \
...
<Note>
For a Google social connection, make sure you enable offline access in the Auth0 Dashboard.
</Note>
...
<td>(Optional) The scopes passed to the external provider as an array of strings.<br/><br/>If used to pass scopes for a Google social connection, include `openid` and `profile` at minimum. At runtime, any scopes included in the `scopes` parameter take precedence over the scopes selected in the Auth0 Dashboard.</td>
</tr>
</tbody>
</table>

```bash lines
curl --request POST "https://${account.namespace}/me/v1/connected-accounts/connect" \
--header 'Content-Type: application/json' \
...
```json
{
  ...
  "connect_uri": "https://${account.namespace}.us.auth0.com/connect",
  "connect_params": {
...
```bash lines
curl --location "https://${account.namespace}/me/v1/connected-accounts/complete" \
--header 'Content-Type: application/json' \
--header "Authorization: Bearer $ME_TOKEN" \
--data '{
...
```bash lines
curl -X GET --location "https://${account.namespace}/me/v1/connected-accounts/connections" \
--header 'Content-Type: application/json' \
--header "Authorization: Bearer <MY_ACCOUNT_API_TOKEN>"
```
...
```bash lines
curl -X GET --location "https://${account.namespace}/me/v1/connected-accounts/accounts" \
--header 'Content-Type: application/json' \
--header "Authorization: Bearer <MY_ACCOUNT_API_TOKEN>"
```
...
```bash lines
curl -X GET --location "https://${account.namespace}/api/v2/users/{userId}/connected-accounts" \
--header 'Content-Type: application/json' \
...
```bash lines
curl -X GET --location "https://${account.namespace}/me/v1/connected-accounts/accounts?connection={connectionName}" \
--header 'Content-Type: application/json' \
--header "Authorization: Bearer <MY_ACCOUNT_API_TOKEN>"
```
...
```bash lines
curl -X DELETE --location "https://${account.namespace}/me/v1/connected-accounts/accounts/{connectedAccountId}" \
--header 'Content-Type: application/json' \
--header "Authorization: Bearer <MY_ACCOUNT_API_TOKEN>"
```
