...
A HAR file is a JSON formatted log of a web browser's interactions with a web server. If authentication isn't working as expected, you can [generate and analyze HAR files](/troubleshoot/troubleshooting-tools/generate-and-analyze-har-files) to find issues.
...
Auth0 stores [log data](/deploy-monitor/logs) including Dashboard administrator actions, successful and failed user authentications, and password change requests. You can view the logs in the [Dashboard](https://manage.auth0.com/#/logs).

Some types of errors do not appear in the logs. For example, if an error occurs at a remote <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/glossary?term=Identity+Provider">Identity Provider</Tooltip>, where authentication doesn’t complete and the user is never returned to Auth0, there will not be any entry in logs.
...
You can also use the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip> to export logs and store them. There are the two available endpoints, each providing slightly different information.
...
* [Verify Rules](/troubleshoot/basic-issues/verify-rules)
