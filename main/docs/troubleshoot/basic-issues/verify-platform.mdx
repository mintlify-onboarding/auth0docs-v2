...
* Check the [Auth0 support matrix](/troubleshoot/customer-support/product-support-matrix) to see if the language or library versions you're using is supported.
...
* [Check the user's profile](/troubleshoot/authentication-issues/check-user-profiles), browser, or device for any issues.
...
* Check to see if the user can access some of the application's key URLs, such as the <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/glossary?term=identity+provider">identity provider</Tooltip>'s <Tooltip tip="Single Sign-On (SSO): Service that, after a user logs into one applicaton, automatically logs that user in to other applications." cta="View Glossary" href="/glossary?term=Single+Sign-on">Single Sign-on</Tooltip> (SSO) URL (indicating a network connectivity issue).
...
* Check notifications in <Tooltip tip="Auth0 Dashboard: Auth0's main product to configure your services." cta="View Glossary" href="/glossary?term=Auth0+dashboard">Auth0 dashboard</Tooltip>/migrations - any changes you overlooked?
