...
---
See the following video on common issues with <Tooltip tip="Custom Domain: Third-party domain with a specialized, or vanity, name." cta="View Glossary" href="/glossary?term=custom+domains">custom domains</Tooltip> and refer to the sections below for troubleshooting steps for specific scenarios.
...
If you see this error when configuring a custom domain, you must perform additional configurations, which varies depending on your setup. To learn more, see [Configure Features to Use Custom Domains](/customize/custom-domains/configure-features-to-use-custom-domains).
...
Alternatively, you can remove reliance on cross-origin authentication by implementing [Auth0 Universal Login](/authenticate/login/auth0-universal-login).
