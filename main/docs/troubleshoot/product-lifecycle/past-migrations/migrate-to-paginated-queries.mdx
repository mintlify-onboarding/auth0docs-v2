...
You may need to update from previous versions of [Auth0 Extensions](/customize/extensions) and custom extensions to the latest versions to make sure they are only performing paginated queries.
...
If you are using the `auth0-deploy-cli` tool, you must update to version **5.3.0 or later**. To learn more, see [Deploy CLI Tool](/deploy-monitor/deploy-cli-tool).