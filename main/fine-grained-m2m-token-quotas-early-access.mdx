...
Fine-grained M2M token quotas are currently in Early Access. Reach out to your Auth0 contact to request access. To learn more about Auth0's product release cycle, read [Product release stages](/troubleshoot/product-lifecycle/product-release-stages).

</Callout>

The [Client Credentials Flow](/get-started/authentication-and-authorization-flow/client-credentials-flow) enables Machine-to-Machine (M2M) applications, such as CLIs, daemons, or backend services, to obtain <Tooltip tip="Access Token: Authorization credential, in the form of an opaque string or JWT, used to access an API." cta="View Glossary" href="/glossary?term=access+tokens">access tokens</Tooltip> on their own behalf without any user interaction.
...
To learn more about rate-limiting headers, read [Predict when requests to a tenant will be rate-limited](/troubleshoot/customer-support/operational-policies/rate-limit-policy/rate-limit-use-cases#predict-when-requests-to-a-tenant-will-be-rate-limited-).
