...
Auth0 makes it easy for your app to implement the Device <Tooltip tip="Authorization Flow: Authorization grant (or workflow) specified in the OAuth 2.0 framework." cta="View Glossary" href="/glossary?term=Authorization+flow">Authorization flow</Tooltip> using:
...
* [Register the Application with Auth0](/get-started/auth0-overview/create-applications/native-apps).
  ...
  + Make sure the Application's **Grant Types** include **Device Code**. To learn how, read [Update Grant Types](/get-started/applications/update-grant-types).
...
* Set up and enable at least one connection for the Application: [Database connections](/get-started/applications/set-up-database-connections), [Social connections](/authenticate/identity-providers/social-identity-providers)
* [Register your API with Auth0](/get-started/architecture-scenarios/mobile-api/part-2#create-the-api)

  + If you want your API to receive Refresh Tokens to allow it to obtain new tokens when the previous ones expire, enable **Allow Offline Access**. To learn more about Refresh Tokens, read [Refresh Tokens](/secure/tokens/refresh-tokens).
...
To get the device code, your app must request a code from the [device code URL](https://auth0.com/docs/api/authentication#get-device-code), including the <Tooltip tip="Client ID: Identification value given to your registered resource from Auth0." cta="View Glossary" href="/glossary?term=Client+ID">Client ID</Tooltip>.
...
* a configured rule denied access (To learn more, read [Auth0 Rules](/customize/rules).)
...
* Checking for active <Tooltip tip="Single Sign-On (SSO): Service that, after a user logs into one applicaton, automatically logs that user in to other applications." cta="View Glossary" href="/glossary?term=SSO">SSO</Tooltip> sessions;
...
Access Tokens are used to call the Auth0 Authentication API's [`/userinfo` endpoint](https://auth0.com/docs/api/authentication#get-user-info) or another API. (To learn more about Access Tokens, read [Access Tokens](/secure/tokens/access-tokens).) You will be able to use the Access Token to call `/userinfo` only if you included the `openid` scope. If you are calling your own API, the first thing your API will need to do is [verify the Access Token](/secure/tokens/access-tokens/validate-access-tokens).

<Tooltip tip="ID Token: Credential meant for the client itself, rather than for accessing a resource." cta="View Glossary" href="/glossary?term=ID+Tokens">ID Tokens</Tooltip> contain user information that must be decoded and extracted. (To learn more about ID Tokens, read [ID Tokens](/secure/tokens/id-tokens).) The `id_token` will only be present in the response if you included the `openid` scope.

<Tooltip tip="Refresh Token: Token used to obtain a renewed Access Token without forcing users to log in again." cta="View Glossary" href="/glossary?term=Refresh+Tokens">Refresh Tokens</Tooltip> are used to obtain a new Access Token or ID Token after the previous one has expired. (To learn more about Refresh Tokens, read [Refresh Tokens](/secure/tokens/refresh-tokens).) The `refresh_token` will only be present in the response if you included the `offline_access` scope and enabled **Allow Offline Access** for your API in the Dashboard.
...
You can use Rules to detect whether the current transaction is using the Device Authorization Flow. (To learn more about rules, read [Auth0 Rules](/customize/rules).) To do so, check the `context` object's `protocol` property:
...
Tenant logs are created for any interaction that takes place and can be used to troubleshoot issues. To learn more, read [Logs](/deploy-monitor/logs).
...
* Support Server Name Indication (SNI) when [Custom Domains](/customize/custom-domains) are used
...
* Be [OIDC-conformant](/get-started/applications/application-settings)
...
* [Social Connections](/authenticate/identity-providers/social-identity-providers) using [Auth0 developer keys](/authenticate/identity-providers/social-identity-providers/devkeys) unless you are using the [Universal Login experience](/authenticate/login/auth0-universal-login/universal-login-vs-classic-login/universal-experience)
...
We support the full Draft 15, except for <Tooltip tip="Confidential Client: A client (application) that can hold credentials securely by using a trusted backend server. Examples include a web application with a secure backend and a machine-to-machine (M2M) application." cta="View Glossary" href="/glossary?term=confidential+Clients">confidential Clients</Tooltip>. To learn more, read [OAuth 2.0 Device Authorization Grant Draft 15 on ietf.org](https://tools.ietf.org/html/draft-ietf-oauth-device-flow-15).
