...
* [リフレッシュトークン](/ja-jp/secure/tokens/refresh-tokens)を使用して新しいIDトークンを取得する。
* IDトークンをサードパーティ（例：Firebase、AWS）のAPIトークンに交換する。

OIDC準拠のパイプラインでは、IDトークンをAPIのセキュリティ保護に使用することができず、リフレッシュトークンは[`/oauth/token`](/ja-jp/api/authentication#refresh-token)エンドポイントで使用する必要があるため、[`/delegation`](/ja-jp/api/authentication#delegation)エンドポイントは廃止されました。
...
* [OIDCでシングルサインオン](/ja-jp/authenticate/login/oidc-conformant-authentication/oidc-adoption-sso)