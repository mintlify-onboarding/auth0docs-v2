...
3. **［Provisioning（プロビジョニング）］** タブを選択し、[ログイン時に追加の属性を同期する](/ja-jp/authenticate/protocols/scim/configure-inbound-scim)場合を除き、 **［Sync user profile attributes at each login（ログインの度にユーザープロファイル属性を同期する）］** を無効にします。
