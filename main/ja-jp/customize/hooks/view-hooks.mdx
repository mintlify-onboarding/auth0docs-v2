...
構成されたフックを表示するには、Dashboardを使用してフックを表示するか、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/ja-jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>を使用してフックの一覧を取得します。
...
[フック取得エンドポイント](/ja-jp/api/management/v2/#!/Hooks/get_hooks)に`GET`呼び出しを行います。`MGMT_API_ACCESS_TOKEN`プレースホルダーの値をManagement APIのアクセストークンで置き換えてください。
...
<td>`create:hooks`<Tooltip id="react-containers-DefinitionTooltip-1">スコープ</Tooltip>を持つManagement APIのアクセストークン。詳細については、<a href="/ja-jp/api/management/v2/tokens">Management APIのアクセストークン</a>をお読みください。</td>
</tr>
</tbody>
</table>