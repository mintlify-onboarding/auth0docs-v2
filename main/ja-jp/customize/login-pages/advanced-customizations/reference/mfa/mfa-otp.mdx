...
The <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="多要素認証 (MFA): ユーザー名とパスワードに加えて、SMS経由のコードなどの要素を使用するユーザー認証プロセス" cta="用語集の表示" href="/ja-jp/glossary?term=multifactor-authentication">MFA</Tooltip> OTP Challenge screen class provides methods associated with the mfa-otp-challenge screen. This screen is shown when a user needs to enter an OTP code during MFA.
