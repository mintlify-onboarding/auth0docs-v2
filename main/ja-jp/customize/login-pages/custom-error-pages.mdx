...
認可エラーが発生し、コールバックURLが有効な場合、認可サーバーは適切なエラーパラメーターと状態パラメーターをコールバックURLに返します。コールバックURLが無効な場合、アプリケーションは[デフォルトのAuth0エラーページ](/ja-jp/authenticate/login/auth0-universal-login/error-pages)を表示します。
...
* ユーザーがブックマークされたログインページに移動し、[デフォルトのログインルート](/ja-jp/authenticate/login/auth0-universal-login/configure-default-login-routes)が指定されていない。
...
<td>`update:tenant_settings`のスコープを持つ<a href="/ja-jp/api/management/v2/tokens">Management APIのアクセストークン</a>。</td>
</tr>
<tr>
<td>`show_log_link`</td>
...
Management APIの[テナント設定更新](/ja-jp/api/management/v2/#!/Tenants/patch_settings)エンドポイントを使用します。`{mgmtApiAccessToken}`プレースホルダー値をManagement APIのアクセストークンに置き換え、JSON本文の`html`フィールドの値をページのHTMLを含む文字列に更新します。
