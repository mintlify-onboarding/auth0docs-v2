...
デフォルトでは、どのアプリケーションのどのユーザーも、ここに定義されているアクセス許可を自由に要求できます。アクセスポリシーを実装すると、[ルール](/ja-jp/customize/rules)を介してこの動作を制限できます。
...
* nickname
* offline_access
* <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" href="/ja-jp/glossary?term=openid" tip="OpenID: アプリケーションがログイン情報を収集および保存することなくにユーザーのIDを検証できるようにする認証用のオープン標準。" cta="用語集の表示">openid</Tooltip>
* 電話
* picture
* profile
