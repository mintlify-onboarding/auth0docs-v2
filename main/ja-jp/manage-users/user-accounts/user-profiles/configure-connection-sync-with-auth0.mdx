...
[ルート属性を個別に](/ja-jp/manage-users/user-accounts/user-profiles/root-attributes/update-root-attributes-for-users)編集するか、Management APIを使用して[一括インポートする](/ja-jp/manage-users/user-accounts/user-profiles/root-attributes/set-root-attributes-during-user-import)ことができます。
...
この手順を完了させる前に、必ず[接続の`options`オブジェクトに既存の値を取得して](/ja-jp/authenticate/identity-providers/retrieve-connection-options)、現在の値がオーバーライドされないようにします。これを行わないと、更新後、元のオブジェクトに欠けているパラメーターが失われてしまいます。
...
* [接続オプションを取得する](/ja-jp/authenticate/identity-providers/retrieve-connection-options)
* [ユーザーのインポート中にルート属性を設定する](/ja-jp/manage-users/user-accounts/user-profiles/root-attributes/set-root-attributes-during-user-import)
...
* [接続をドメインレベルに昇格する](/ja-jp/authenticate/identity-providers/promote-connections-to-domain-level)