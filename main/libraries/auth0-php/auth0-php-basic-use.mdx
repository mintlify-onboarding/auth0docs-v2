...
For additional information on these capabilities and more, please see the [documentation page for the Authentication API](/libraries/auth0-php/using-the-authentication-api-with-auth0-php).

## Prerequisites

The documentation below assumes that you followed the steps in the [Auth0-PHP](/libraries/auth0-php), and continue off from the code provided there.

### Logging In

The default login process in the PHP SDK uses an [Authentication Code grant](/get-started/authentication-and-authorization-flow/authorization-code-flow/call-your-api-using-the-authorization-code-flow) combined with Auth0's <Tooltip tip="Universal Login: Your application redirects to Universal Login, hosted on Auth0's Authorization Server, to verify a user's identity." cta="View Glossary" href="/glossary?term=Universal+Login">Universal Login</Tooltip> Page. In short, that process is:

1. A user requesting access is redirected to the Universal Login Page.
2. The user authenticates using one of [many possible connections](/authenticate/identity-providers): social (Google, X, Facebook), database (email and password), passwordless (email, SMS), or enterprise (ActiveDirectory, ADFS, Office 365).
...
If you're using <Tooltip tip="Single Sign-On (SSO): Service that, after a user logs into one applicaton, automatically logs that user in to other applications." cta="View Glossary" href="/glossary?term=Single+Sign-on">Single Sign-on</Tooltip> (SSO) and also want to end their Auth0 session, see the [SSO Logout section here](/libraries/auth0-php/using-the-authentication-api-with-auth0-php). More information about logging out, in general, can be found [here](/authenticate/login/logout).

## Learn more

* [PHP: Getting Started using Auth0-PHP](/libraries/auth0-php)
* [PHP: Using the Authentication API with Auth0-PHP](/libraries/auth0-php/using-the-authentication-api-with-auth0-php)
