...
| `MGMT_API_ACCESS_TOKEN` | [Access Token for the Management API](/secure/tokens/access-tokens/management-api-access-tokens) with the scope `create:organization_invitations`. |
...
| `400` | `invalid_body` | A default login route is required to generate the invitation url. To learn more, see [Configure default login routes](/authenticate/login/auth0-universal-login/configure-default-login-routes). | The request payload is not valid. |
