...
1. Log the user into your application.<br />
   ...
   The user authenticates to your application using [Universal Login](/authenticate/login/auth0-universal-login). To learn more, read the [Regular Web App Quickstart](/quickstart/webapp), asking for a token for the Auth0 Management API audience (audience=`https://{yourDomain}/api/v2/`).
...
5. Your application calls the Auth0 Management API's [Link a User Account endpoint](https://auth0.com/docs/api/v2#!/Users/post_identities) using a [Management API Access Token](/secure/tokens/access-tokens/management-api-access-tokens) with the `update:users` scope.
