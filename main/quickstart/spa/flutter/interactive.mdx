...
<Recipe>
  <Content>
    ...
    <Section id={sections[2].id} title={sections[2].title} stepNumber="3">
      [Universal Login](/authenticate/login/auth0-universal-login) is
      ...
      You will normally need to specify the `redirectUrl` parameter to
                      `loginWithRedirect`. Omitting this will cause Auth0 to use the [default login route](/authenticate/login/auth0-universal-login/configure-default-login-routes), which is not configured by default.
      </Info>
    ...
    <Section id={sections[3].id} title={sections[3].title} stepNumber="4">
      To log users out, redirect them to the Auth0 logout endpoint to clear their login session by calling the Auth0
            Flutter SDK `logout()`. [Read
              more about logging out of Auth0](/authenticate/login/logout).

      <Info>
      You will normally want to specify `returnToUrl` when calling `logout`, otherwise
                      Auth0 [will default to the first URL in the Allowed Logout URLs list](/authenticate/login/logout/redirect-users-after-logout).
      </Info>
