...
* **Control Your Own Key:** Allows users with the  [Key Management Editor role](/get-started/manage-dashboard-access/add-dashboard-users) to customize the lifecycle of the Tenant Master Key in Auth0 KMS.
* **Bring Your Own Key:** Allows users with the  [Key Management Editor role](/get-started/manage-dashboard-access/add-dashboard-users) to replace the Auth0 Environment Root Key and import their own wrapped encryption key to the corresponding Auth0 Cloud Hardware Security Module (HSM).

To learn more about Auth0 Keys, [read Auth0 key hierarchy](/secure/highly-regulated-identity/customer-managed-keys#auth0-key-hierarchy).

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">

The Customer Managed Keys feature is available to users with the Key Management Editor role. This role is not applied to users by default and needs to be explicitly applied to a Tenant member. To learn more, read [Add Tenant Members](/get-started/manage-dashboard-access/add-dashboard-users).

</Callout>

## Control Your Own Key

You can control the lifecycle of your Tenant Master Key using the Auth0 <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip> [Rekey](https://auth0.com/docs/api/management/v2/keys/post-encryption-rekey) endpoint to:
...
The endpoint is only available to tenant members with specific authorization scopes in Auth0, by default these scopes are granted to users with the  [Key Management Editor role](/get-started/manage-dashboard-access/add-dashboard-users).
...
You can use the [Dashboard](/secure/highly-regulated-identity/customer-managed-keys/customer-managed-keys-dashboard) or the [Management API](/secure/highly-regulated-identity/customer-managed-keys/customer-managed-keys-management-api) to enable Bring Your Own Key.
...
The Customer Managed Keys operations adds the following log event in your [tenant logs](/deploy-monitor/logs):
