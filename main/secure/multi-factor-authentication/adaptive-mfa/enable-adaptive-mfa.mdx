...
2. In the **Factors** section, enable and configure at least one MFA Factor. To learn more, read [Multi-Factor Authentication Factors](/secure/multi-factor-authentication/multi-factor-authentication-factors).
...
1. Get a [Management API access token](/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) with the `update:mfa_policies` scope.
...
You can customize the behavior of Adaptive MFA to provide the best experience for your users while ensuring security. To learn more, read [Customize Adaptive MFA](/secure/multi-factor-authentication/adaptive-mfa/customize-adaptive-mfa).
...
Assessment information in tenant logs is only available for interactive flows. Auth0 does not support recording assessment information for <Tooltip tip="Resource Owner: Entity (such as a user or application) capable of granting access to a protected resource." cta="View Glossary" href="/glossary?term=Resource+Owner">Resource Owner</Tooltip> Password Grant (ROPG) flows without adaptive MFA enabled. For more information about authentication flow limitations, read [Adaptive MFA](/secure/multi-factor-authentication/adaptive-mfa).
