...
* [Save and Renew Tokens for Android](/auth0-android-save-and-renew-tokens)
* [Save and Renew Tokens for Swift](/libraries/auth0-swift/auth0-swift-save-and-renew-tokens)
* [Native/Mobile Apps Quickstarts](/quickstart/native)

</Tab><Tab title="Single-page app">

We recommend using the [Auth0 SPA SDK](/libraries/auth0-single-page-app-sdk) to handle token storage, session management, and other details for you.
...
In compliance with the OAuth2 specifications, when a browser requests a refresh token from the /token endpoint, Auth0 will only return a Refresh Token if [Refresh Token Rotation](/secure/tokens/refresh-tokens/refresh-token-rotation) is enabled for that client.
...
Using browser local storage can be a viable alternative to mechanisms that require retrieving the <Tooltip tip="Access Token: Authorization credential, in the form of an opaque string or JWT, used to access an API." cta="View Glossary" href="/glossary?term=access+token">access token</Tooltip> from an iframe and to cookie-based authentication across domains when these are not possible due to browser restrictions (for example, ITP2).
