...
---
Auth0 limits the use of services to assure optimal performance and protection from <Tooltip tip="Bad Actors: Entity (a person or group) that poses a threat to the business or environment with the intention to cause harm." cta="View Glossary" href="/glossary?term=bad+actors">bad actors</Tooltip>, technical errors, or excessive legitimate traffic.  We recommend you review how Auth0 enforces limits to configure your application for the best user experience.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">

To find rate limits associated with your account, see [Rate Limit Configurations](/troubleshoot/customer-support/operational-policies/rate-limit-policy/rate-limit-configurations) for a matrix of all rate limit policies.
...
In Private Cloud, environment request limits are based on the Private Cloud Performance Tier.  To learn more, read [Private Cloud for AWS](/deploy-monitor/deploy-private-cloud/private-cloud-on-aws) or [Private Cloud for Azure](/deploy-monitor/deploy-private-cloud/private-cloud-on-azure).
...
For Private Cloud load testing considerations, refer to [Private Cloud on AWS](/deploy-monitor/deploy-private-cloud/private-cloud-on-aws) and [Private Cloud on Azure](/deploy-monitor/deploy-private-cloud/private-cloud-on-azure).
...
For example, a free, non-production tenant could have different limits than a production tenant with a paid subscription. For specific rate limit configurations, see [Rate Limit Configurations](/troubleshoot/customer-support/operational-policies/rate-limit-policy/rate-limit-configurations).
...
Customers leveraging Extensibility could add still more requests, not only to the Authentication API, but also the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/glossary?term=Management+API">Management API</Tooltip>, depending on the Extensibility Configuration.
See [Rate Limit Use Cases](/troubleshoot/customer-support/operational-policies/rate-limit-policy/rate-limit-use-cases) for guidance on how to estimate how your Auth0 configuration can impact your API usage.
...
Auth0's [brute-force protection](/secure/attack-protection/brute-force-protection) and [suspicious IP throttling](/secure/attack-protection/suspicious-ip-throttling) can also limit logins and signups, but are independent of rate limits. To learn more about how Auth0 detects and handles potentially malicious anomalies, read [Attack Protection](/secure/attack-protection).
...
Monitoring of the interval deduction events is possible through [tenant logs](/deploy-monitor/logs/log-event-type-codes). Each deduction generates an associated `appi` tenant log event type containing information about the already consumed and remaining allowance.

For more information, see the Authentication API section in [Rate Limits - Enterprise](/troubleshoot/customer-support/operational-policies/rate-limit-policy/rate-limit-configurations/enterprise-public).
...
* [Rate Limit Use Cases](/troubleshoot/customer-support/operational-policies/rate-limit-policy/rate-limit-use-cases)