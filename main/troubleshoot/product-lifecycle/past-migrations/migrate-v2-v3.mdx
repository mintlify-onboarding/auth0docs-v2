...
---
User search v2 has reached its end of life as of **June 30, 2019**. We highly recommend migrating user search functionality to [search engine v3](/manage-users/user-search) (`search_engine=v3`) as soon as possible.
...
<tr>
<td><a href="/customize/extensions/delegated-administration-extension">Delegated Administration</a></td>
<td>3.1+</td>
...
You can leverage the [logs](/deploy-monitor/logs) in the [Dashboard](https://manage.auth0.com/#/logs) to find calls to the `/api/v2/users` endpoint that use the User Search v2 engine, including calls performed by SDKs. Those logs will help you identify where code changes might be needed in your applications.
